    ...
    <div className="theme-kinetic bg-background text-on-background font-body-md selection:bg-secondary selection:text-white" dir={dir}>
          ...
          <div className="flex items-center gap-4">
            {profile.phone && (
              <a className="hidden md:flex items-center gap-2 px-4 py-2 bg-whatsapp-green text-white rounded-lg font-bold hover:scale-95 transition-transform active:scale-90"
                ...
                <span className="font-arabic-ui text-arabic-ui">{k.whatsappSupport}</span>
              </a>
            )}
            {showLangToggle && (
      ...
      <main>
          ...
          <div className="relative z-10 w-full max-w-screen-2xl mx-auto px-margin-desktop py-section-gap grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              ...
              <p className="text-xl text-primary-fixed max-w-lg font-body-md">
                {profile.heroSubtitle ?? k.salesHeroSubtitle}
              </p>
            ...
            <div className="hidden lg:block relative group">
              ...
              <div className="absolute bottom-6 right-6 z-20 glass-card p-6 rounded-xl border border-white/20">
                <p className="text-primary font-bold text-lg mb-1">{k.weeklySpecial}</p>
                <p className="text-secondary text-3xl font-extrabold">{k.askAboutDeals}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-section-gap px-margin-desktop max-w-screen-2xl mx-auto">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="font-headline-lg text-headline-lg text-primary uppercase italic">{k.hotOffers}</h2>
        ...
        <section className="bg-primary-container text-white py-section-gap relative overflow-hidden">
          ...
          <div className="max-w-screen-2xl mx-auto px-margin-desktop grid lg:grid-cols-2 gap-16 items-center relative z-10">
            <div>
              <h2 className="font-headline-lg text-headline-lg mb-6 uppercase">{k.estimateInstallmentsTitle} <span className="text-secondary">{k.inTenSeconds}</span></h2>
              <p className="text-on-primary-container text-lg mb-10 max-w-md">
                {k.instantEstimateDesc}
              </p>
            ...
            <div className="bg-white rounded-2xl p-8 text-primary shadow-2xl">
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-bold uppercase tracking-wider text-outline mb-2">{k.carPriceLabel}</label>
                ...
                <div>
                  <label className="block text-sm font-bold uppercase tracking-wider text-outline mb-2">{k.downPaymentPercentLabel}</label>
                ...
                <p className="text-[10px] text-outline text-center">{k.financeTermsNote}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-secondary py-16 px-margin-desktop text-white text-center">
          <h2 className="text-4xl md:text-5xl font-black uppercase italic mb-8">{k.readyToDriveDream}</h2>
      ...
      <footer className="bg-primary py-section-gap">
        <div className="w-full max-w-screen-2xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-gutter px-margin-desktop text-on-primary">
          ...
          <div>
            <h4 className="font-bold text-white mb-6 uppercase tracking-widest">{k.quickLinks}</h4>
            <ul className="space-y-4 text-on-primary-container">
              ...
              <li><Link className="hover:text-luxury-gold transition-colors" href="/branches">{k.locations}</Link></li>
            </ul>
          </div>
          <div>
          ...
          <div>
            <h4 className="font-bold text-white mb-6 uppercase tracking-widest">{k.salesFooterGetInTouch}</h4>
            <p className="text-on-primary-container text-sm mb-4">{k.salesFooterGetInTouchDesc}</p>
            <Link href="/contact" className="inline-block bg-secondary text-white px-4 py-2 rounded-lg font-bold">{k.contactUsButton}</Link>
          </div>
        </div>
