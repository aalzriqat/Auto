...
describe("Phase 9 — expense account mapping", () => {
  test("general expense posts to General Expenses (6300), not Commission Expense (6100)", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-accountingPhase9.test-113-66",
  ...
  test("bank-transfer expense stores method and credits Bank", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-accountingPhase9.test-129-66",
      orgId,
...
describe("Phase 9 — accounting outbox", () => {
  test("event with no open period is enqueued, then posts when a period opens", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-accountingPhase9.test-269-66",
  ...
  test("an event that keeps failing moves to FAILED after 10 attempts and stops being drained", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-accountingPhase9.test-302-66",
  ...
  test("an event waiting for its own period is not dead-lettered by other periods opening", async () => {
    ...
    const decemberExpenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-accountingPhase9.test-338-74",
  ...
  test("retryFailed resets a FAILED event back to PENDING for another drain attempt", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-accountingPhase9.test-383-66",
  ...
  test("retryFailed rejects a non-FAILED event", async () => {
    ...
    await asUser.mutation(api.expenses.create, { idempotencyKey: "t-accountingPhase9.test-413-48",
...
describe("Phase 9 — reversal audit log", () => {
  test("reversing an event writes a REVERSE_EVENT audit entry", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-accountingPhase9.test-462-66",
...
describe("payment intent settlement clamping", () => {
  test("settles even when the receivable was partly paid through another channel", async () => {
    ...
    const intentId = await asUser.mutation(api.paymentIntents.create, { idempotencyKey: "t-accountingPhase9.test-745-71",
      orgId,
      customerId,
