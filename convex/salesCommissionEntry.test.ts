...
async function completedSale(
  ...
  return await ids.asAdmin.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
    orgId: ids.orgId,
...
describe("MANUAL mode: the commissions page is an entry point, not a dead end", () => {
  ...
  test("a commission accrued by an approved payroll run is corrected, not frozen", async () => {
    ...
    const saleId = await ids.asAdmin.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
      orgId: ids.orgId,
      vehicleId: ids.vehicleId,
      customerId: ids.customerId,
      salespersonId: ids.userId,
  ...
  test("an older undecided sale is still reachable behind pages of decided ones", async () => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      saleIds.push(
        await ids.asAdmin.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
          orgId: ids.orgId,
  ...
  test("a paid commission leaves the editable set", async () => {
    ...
    await ids.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: crypto.randomUUID(),
      orgId: ids.orgId,
      saleId,
      paymentMethod: "CASH",
    });
  ...
  test("an undecided row cannot be marked paid — there is nothing to pay", async () => {
    ...
    await expect(
      ids.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: crypto.randomUUID(),
        orgId: ids.orgId,
        saleId,
        paymentMethod: "CASH",
      })
