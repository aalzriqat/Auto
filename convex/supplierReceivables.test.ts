...
async function seed(tag: string) {
  ...
  const saleId = await asUser.mutation(api.sales.create, { idempotencyKey: "t-supplierReceivables.test-97-58",
...
describe("the claim a direct-settled sale opens", () => {
  ...
  test("a through-dealership sale opens no claim, because the money ran the other way", async () => {
    ...
    const saleId = await s.asUser.mutation(api.sales.create, { idempotencyKey: "t-supplierReceivables.test-162-62",
...
describe("collecting it", () => {
  test("a part payment moves the subledger and the ledger by the same amount", async () => {
    ...
    const result = await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-183-83",
      orgId: s.orgId,
  ...
  test("instalments accumulate and the last one settles the claim to zero", async () => {
    ...
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-208-68",
    ...
    // is for.
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-214-68",
      orgId: s.orgId, receivableId: s.receivable._id, amount: 1_000, receiptMethod: "CASH",
    });
    const final = await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-217-82",
      orgId: s.orgId, receivableId: s.receivable._id, amount: 1_000, receiptMethod: "CASH",
    });
  ...
  test("collecting more than is owed is refused rather than absorbed", async () => {
    ...
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-236-68",
      orgId: s.orgId, receivableId: s.receivable._id, amount: 2_500, receiptMethod: "CASH",
    });
    await expect(
      s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-240-64",
        orgId: s.orgId, receivableId: s.receivable._id, amount: 1_000, receiptMethod: "CASH",
      })
  ...
  test("a zero or negative receipt is refused", async () => {
    ...
    for (const amount of [0, -100]) {
      await expect(
        s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-254-66",
  ...
  test("a settled claim cannot be collected against again", async () => {
    ...
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-263-68",
      orgId: s.orgId, receivableId: s.receivable._id, amount: MARGIN, receiptMethod: "CASH",
    });
    await expect(
      s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-267-64",
...
describe("disputes", () => {
  test("a disputed claim refuses money until the dispute is lifted", async () => {
    ...
    await expect(
      s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-283-64",
...
describe("cancellation", () => {
  ...
  test("cancelling is refused once the supplier has paid something", async () => {
    ...
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-324-68",
      orgId: s.orgId, receivableId: s.receivable._id, amount: 500, receiptMethod: "CASH",
    });
...
describe("what a supplier still owes", () => {
  test("the outstanding summary totals the claims the GL balance cannot break down", async () => {
    ...
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-342-68",
      orgId: s.orgId, receivableId: s.receivable._id, amount: 1_000, receiptMethod: "CASH",
    });
...
describe("tenancy", () => {
  test("another organization's claim is not readable or collectable", async () => {
    ...
    await expect(
      asOther.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-supplierReceivables.test-387-63",
...
describe("a deposit applied to the settlement", () => {
  test("opens the claim net of it, so subledger and ledger agree from the first moment", async () => {
    ...
    const saleId = await base.asUser.mutation(api.sales.create, { idempotencyKey: "t-supplierReceivables.test-435-65",
