...
describe("expenses.create", () => {
  test("creates an expense and posts a ledger transaction", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-37-66",
      orgId,
      title: "Office Rent",
      amount: 5000,
      date: expenseDate,
  ...
  test("rejects unauthenticated requests", async () => {
    ...
    await expect(
      t.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-69-39",
        orgId,
        title: "Test",
        amount: 100,
        date: Date.now(),
  ...
  test("rejects vehicle that does not belong to the org", async () => {
    ...
    await expect(
      asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-102-44",
        orgId,
  ...
  test("creates expense linked to a vehicle", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-132-66",
      orgId,
      vehicleId,
      title: "Oil Change",
      amount: 150,
  ...
  test("creates PENDING expense without cash transaction or accounting event", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-157-66",
      orgId,
...
describe("expenses.update", () => {
  test("marking a PENDING expense PAID records the cash transaction and accounting event", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-192-66",
      orgId,
  ...
  test("rejects accounting field changes after an expense is posted", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-234-66",
      orgId,
      title: "Utility Bill",
      amount: 300,
      date: originalDate,
...
describe("Phase 2 — amortization start date can't predate the expense's month", () => {
  test("create rejects a start date in an earlier calendar month than the expense", async () => {
    ...
    await expect(
      asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-272-44",
  ...
  test("create accepts a start date earlier in the SAME calendar month as the expense (month-level, not day-level)", async () => {
    const { orgId, asUser } = await setup();
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-282-66",
  ...
  test("create accepts a start date after the expense's month (coverage begins later)", async () => {
    const { orgId, asUser } = await setup();
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-292-66",
  ...
  test("update rejects a start date earlier than the (unchanged) expense's month", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-304-66",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 2, 1),
  ...
  test("update rejects moving the expense date later than an already-set start date", async () => {
    const { orgId, asUser } = await setup();
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-317-66",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 2, 1),
      category: "FEES", status: "PENDING", isPrepaid: true, amortizationMonths: 12,
  ...
  test("update accepts null to explicitly clear a previously-set start date (distinct from omitting the field)", async () => {
    const { t, orgId, asUser } = await setup();
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-331-66",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 2, 1),
      category: "FEES", status: "PENDING", isPrepaid: true, amortizationMonths: 12,
  ...
  test("update omitting amortizationStartDate leaves an existing value untouched (undefined means 'no change', not 'clear')", async () => {
    const { t, orgId, asUser } = await setup();
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-345-66",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 2, 1),
      category: "FEES", status: "PENDING", isPrepaid: true, amortizationMonths: 12,
...
describe("expenses.remove", () => {
  test("rejects deletion after an expense is posted", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-362-66",
      orgId,
      title: "Delete Me",
      amount: 200,
      date: Date.now(),
  ...
  test("leaves the linked transaction row active when deletion is rejected", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-383-66",
      orgId,
...
describe("expenses.reverseExpense", () => {
  ...
  test("cancels a queued (not yet actually posted) expense post and removes the expense", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-450-66",
      orgId,
      title: "No chart yet",
      amount: 50,
      date: Date.now(),
  ...
  test("reverses a fully posted expense with a real offsetting journal entry", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-477-66",
      orgId,
      title: "Fully posted",
      amount: 75,
      date: Date.now(),
  ...
  test("requires a non-empty reason", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-515-66",
      orgId,
  ...
  test("a user without manage:finance cannot reverse an expense", async () => {
    ...
    const expenseId = await asFinanceUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-531-73",
      orgId,
      title: "Restricted",
      amount: 20,
      date: Date.now(),
...
describe("expenses VAT split (Phase 41)", () => {
  ...
  test("an expense with a VAT amount splits the ledger into net expense + VAT receivable", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-593-66",
      orgId,
  ...
  test("an expense without a VAT amount posts the plain two-line entry (unchanged behavior)", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-638-66",
      orgId,
      title: "No VAT",
      amount: 200,
      date: Date.now(),
  ...
  test("a MARKETING expense debits the dedicated Marketing Expense account, not GENERAL_EXPENSE", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-665-66",
      orgId,
  ...
  test("self-heals a missing category account for a chart initialized before this addition", async () => {
    ...
    const expenseId = await asUser.mutation(api.expenses.create, { idempotencyKey: "t-expenses.test-712-66",
      orgId,
      title: "Showroom rent",
      amount: 900,
      date: Date.now(),
