...
const directSale = async (
  ...
  (await seed.asUser.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
    orgId: seed.orgId,
    vehicleId,
    customerId,
...
describe("S.4 an organization without the canonical authority", () => {
  ...
  test("S.4d the withheld contract holds through api.applications.cancelApplication", async () => {
    ...
    await cancelAndDrain(seed, period._id, () =>
      seed.asManager.mutation(api.applications.cancelApplication, { idempotencyKey: crypto.randomUUID(),
        orgId: seed.orgId,
        applicationId,
