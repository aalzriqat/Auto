...
describe("P1-M1 the acting root is decided in exactly one place", () => {
  test("1.1 a FREE car acquires exactly ONE root and ONE episode (I1)", async () => {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-288-53", orgId: seed.orgId, quoteId, amount: 2_000 });
  ...
  test("1.2 the SAME customer opening a SECOND independent deal is REFUSED (I2)", async () => {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-307-53",
      orgId: seed.orgId,
      quoteId: first,
      amount: 2_000,
    });
    ...
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-315-49",
        orgId: seed.orgId,
        quoteId: second,
        amount: 1_000,
      }),
  ...
  test("1.3 a DIFFERENT customer is refused, and nothing moves", async () => {
    ...
    const v = await vehicle(seed);
    const first = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-330-53",
      orgId: seed.orgId,
      quoteId: first,
      amount: 2_000,
    });
    ...
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-339-49", orgId: seed.orgId, quoteId: rival, amount: 500 }),
      HELD,
      "1.3"
    );
  ...
  test("1.4 the SAME deal taking a further instalment JOINS its root (I3)", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-357-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-358-53", orgId: seed.orgId, quoteId, amount: 1_000 });
  ...
  test("1.5 REFUSE is a decision — a held car never falls through to a new root", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-385-53", orgId: seed.orgId, quoteId, amount: 2_000 });
  ...
  test("1.6 two OPEN roots REFUSE rather than pick a winner", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-421-53", orgId: seed.orgId, quoteId, amount: 2_000 });
  ...
  test("1.7 another organization's root does not hold THIS org's car", async () => {
    ...
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-473-53", orgId: seed.orgId, quoteId, amount: 2_000 });
...
describe("P1-M2 evidence is tagged and carried", () => {
  ...
  test("2.2 a DEPOSIT acquisition is tagged DEPOSIT and names its deposit", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-520-53", orgId: seed.orgId, quoteId, amount: 2_000 });
  ...
  test("2.4 a successor may not move root, and may not succeed a LIVE episode (I6)", async () => {
    ...
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-591-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-615-53",
      orgId: seed.orgId,
      quoteId: otherQuote,
      amount: 1_000,
    });
...
describe("P1-W the five reachable acquisition writers", () => {
  ...
  async function releasedSlice(seed: Seed, a: Id<"vehicles">, b: Id<"vehicles">) {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-665-53", orgId: seed.orgId, quoteId, amount: 6_000 });
  ...
  test("W.5 a deposit then an application on ONE deal is two episodes on ONE root", async () => {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-869-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    await seed.asUser.mutation(api.applications.createFromQuote, { orgId: seed.orgId, quoteId });
  ...
  test("W.6 a finance application REFUSES a car another deal holds", async () => {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-889-53",
      orgId: seed.orgId,
      quoteId: held,
      amount: 2_000,
    });
...
describe("P1-A continuation is proven, never inferred", () => {
  ...
  test("A.1 a deposit ADOPTS the reservation it names — one root, no second deal", async () => {
    ...
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-946-53",
      orgId: seed.orgId,
      quoteId,
      amount: 2_000,
  ...
  test("A.2 once adopted, later doors prove lineage by QUOTE — adoption happens once", async () => {
    ...
    const { v, reservationId } = await reserved(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-972-53",
      orgId: seed.orgId,
      quoteId,
      amount: 2_000,
      adoptReservationId: reservationId,
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-982-53",
      orgId: seed.orgId,
      quoteId,
      amount: 500,
    });
  ...
  test("A.3 adoption is REFUSED when the named reservation is not the one holding the car", async () => {
    ...
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1007-49",
        orgId: seed.orgId,
        quoteId,
        amount: 2_000,
  ...
  test("A.4 a RIVAL cannot adopt a reservation that is not theirs", async () => {
    ...
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1038-49",
        orgId: seed.orgId,
        quoteId: rivalQuote,
        amount: 1_000,
        adoptReservationId: reservationId,
  ...
  test("A.4b the SAME fixture SUCCEEDS for the reservation's own customer", async () => {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1075-53",
      orgId: seed.orgId,
      quoteId: ownQuote,
      amount: 2_000,
      adoptReservationId: reservationId,
    ...
    // repeats.
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1092-53",
      orgId: seed.orgId,
      quoteId: ownQuote,
      amount: 500,
    });
  ...
  test("A.5 a car THIS deal's deposit holds can still be reserved, on named proof", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1104-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    const rootBefore = (await rootsOn(seed, v))[0];
  ...
  test("A.6 without that proof, reserving a held car is still refused", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1130-53", orgId: seed.orgId, quoteId, amount: 2_000 });
  ...
  async function multiVehicleDeposit(seed: Seed, cars: Id<"vehicles">[], amount = 4_000) {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1179-53", orgId: seed.orgId, quoteId, amount });
    return await seed.t.run(async (ctx) => {
  ...
  test("A.7e a pointer at ANOTHER DEPOSIT's episode REFUSES", async () => {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1347-53",
      orgId: seed.orgId,
  ...
  test("A.8 a reservation from ANOTHER TENANT refuses even though the quote alone would JOIN", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1521-53", orgId: seed.orgId, quoteId, amount: 2_000 });

    const before = await snapshot(seed, v);
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1525-49",
        orgId: seed.orgId,
        quoteId,
        amount: 1_000,
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1538-53", orgId: seed.orgId, quoteId, amount: 1_000 });
  ...
  test("A.9 an INACTIVE reservation refuses even though the quote alone would JOIN", async () => {
    ...
    await seed.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-commitmentAuthority.test-1554-68",
      orgId: seed.orgId,
      applicationId,
    ...
    const before = await snapshot(seed, v);
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1564-49",
        orgId: seed.orgId,
        quoteId,
        amount: 2_000,
        adoptReservationId: reservationId,
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1590-53", orgId: seed.orgId, quoteId, amount: 2_000 });
  ...
  test("A.10 a reservation for a car this quote does NOT cover refuses", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1606-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    ...
    const before = await snapshot(seed, v);
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1621-49",
        orgId: seed.orgId,
        quoteId,
        amount: 1_000,
        adoptReservationId: otherReservation,
  ...
  test("A.12 adoption REFUSES a reservation that did not open this root", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1646-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    ...
    const before = await snapshot(seed, v);
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1661-49",
        orgId: seed.orgId,
        quoteId,
        amount: 1_000,
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1674-53", orgId: seed.orgId, quoteId, amount: 1_000 });
    expect((await rootsOn(seed, v)).length, "the quote alone really would have joined").toBe(1);
  });
  ...
  test("A.14 the AUTHORITY refuses a foreign customer reusing another deal's deposit proof", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1715-53", orgId: seed.orgId, quoteId, amount: 2_000 });
  ...
  test("A.15 and on the public path that attempt leaves no residue", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1756-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    const depositId = await seed.t.run(async (ctx) => {
      const rows = await ctx.db.query("deposits").collect();
      return rows.find((d) => d.vehicleId === v)!._id;
    });
  ...
  test("A.18 a held car refuses a DEPOSIT proof from an unattributed operation", async () => {
    ...
    const v = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1856-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    const depositId = await seed.t.run(async (ctx) => {
      const rows = await ctx.db.query("deposits").collect();
      return rows.find((d) => d.vehicleId === v)!._id;
    });
  ...
  test("A.19 attribution alone still grants NOTHING — lineage must prove the root", async () => {
    ...
    const v = await vehicle(seed);
    const first = await cashQuote(seed, seed.customerA, v);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1886-53",
      orgId: seed.orgId,
      quoteId: first,
      amount: 2_000,
    });
  ...
  test("A.11 a MULTI-VEHICLE deal continuing one reservation is not broken by it", async () => {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-1952-53",
      orgId: seed.orgId,
      quoteId,
      amount: 4_000,
      adoptReservationId: reservationId,
...
describe("P1-C2 a supplied lineage proof is proved, not taken on trust", () => {
  ...
  test("C2.5 a deposit that does not apply to this car refuses", async () => {
    ...
    const free = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, otherCar);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-2101-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    const depositId = await seed.t.run(async (ctx) => {
      const rows = await ctx.db.query("deposits").collect();
  ...
  test("C2.6 a deposit belonging to ANOTHER customer refuses, and its owner's does not", async () => {
    ...
    const free = await vehicle(seed);
    const quoteId = await cashQuote(seed, seed.customerA, otherCar);
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-2134-53", orgId: seed.orgId, quoteId, amount: 2_000 });
    const depositId = await seed.t.run(async (ctx) => {
      const rows = await ctx.db.query("deposits").collect();
      return rows.find((d) => String(d.vehicleId) === String(otherCar))!._id;
    });
...
describe("P1-S the multi-vehicle narrowing refuses what it must", () => {
  test("S.5 a legitimate adoption does not carry the deal onto a car ANOTHER deal holds", async () => {
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-2515-53",
      orgId: seed.orgId,
      quoteId: rivalQuote,
      amount: 2_000,
    });
    ...
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-2542-49",
        orgId: seed.orgId,
        quoteId,
        amount: 4_000,
        adoptReservationId: reservationId,
  ...
  test("S.6 a FREE car refuses an invalid adoption — validation precedes opening", async () => {
    ...
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-2578-49",
        orgId: seed.orgId,
        quoteId,
        amount: 2_000,
    ...
    await seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-2600-53",
      orgId: seed.orgId,
      quoteId,
      amount: 2_000,
    });
...
describe("P1-C2E a reservation proof must still be LIVE, not merely ACTIVE", () => {
  ...
  test("C2E.5 and the PUBLIC adoption door refuses it, leaving no residue", async () => {
    ...
    await expectRefusal(
      seed.asUser.mutation(api.deposits.create, { idempotencyKey: "t-commitmentAuthority.test-2799-49",
        orgId: seed.orgId,
        quoteId,
        amount: 2_000,
        adoptReservationId: reservationId,
