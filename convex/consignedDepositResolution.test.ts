...
async function seed(
  ...
  if (instalments) {
    for (const amount of instalments) {
      depositIds.push(
        await asUser.mutation(api.deposits.create, { idempotencyKey: crypto.randomUUID(), orgId, quoteId, amount, method: "CASH" })
      );
    }
...
async function completeAs(
  ...
  return await actor.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
    orgId: s.orgId,
...
describe("a quote-wide treatment on a quote whose lines are not all consigned", () => {
  test("completes both lines: the treatment means the same thing on owned stock", async () => {
    ...
    const saleIds = await s.asUser.mutation(api.sales.completeFromQuote, { idempotencyKey: crypto.randomUUID(),
      orgId: s.orgId,
      quoteId: s.quoteId,
