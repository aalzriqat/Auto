...
describe("recording instalments against a supplier", () => {
  test("two instalments settle the payable, and the balance falls as they land", async () => {
    const s = await seed();
    const first = await s.asUser.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-supplierSettlement.test-113-86",
    ...
    const second = await s.asUser.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-supplierSettlement.test-119-87",
  ...
  test("refuses to pay a supplier more than he is owed", async () => {
    const s = await seed();
    await s.asUser.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-supplierSettlement.test-133-72",
      orgId: s.orgId, payableId: s.payableId, amount: 9_000,
    });
    // Netting the excess into the next payable would hide which deal it
    // happened on.
    await expect(
      s.asUser.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-supplierSettlement.test-139-68",
  ...
  test("a disputed payable cannot be settled by quietly paying it", async () => {
    ...
    await expect(
      s.asUser.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-supplierSettlement.test-163-68",
  ...
  test("lifting a dispute returns to what the money says, not to a remembered status", async () => {
    const s = await seed();
    await s.asUser.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-supplierSettlement.test-181-72",
      orgId: s.orgId, payableId: s.payableId, amount: 4_000,
    });
    await s.asUser.mutation(api.sourcingPayables.setDisputed, {
  ...
  test("neither mutation will touch another organization's payable", async () => {
    ...
    await expect(
      s.asUser.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-supplierSettlement.test-219-68",
  ...
  test("settling in full records the amount, not just the flag", async () => {
    const s = await seed();
    await s.asUser.mutation(api.sourcingPayables.markPaid, { idempotencyKey: "t-supplierSettlement.test-237-60",
