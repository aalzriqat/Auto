...
describe("SCRUM-121 Priority-1 round 5 — path × input shape, measured on clean main", () => {
  ...
  test("L2 · clear then return with lineage missing: atomic refusal, nothing survives", async () => {
    ...
    // R8/R12.6 — the pre-return world survives untouched. `rowOutstandingMinor: 0`
    ...
    //
    // ⚠️ THE REFUSAL MUST BE A DELIBERATE ONE, AND THIS ASSERTION EXISTS BECAUSE
    // A MUTANT SURVIVED WITHOUT IT. Deleting the lineage guard still "threw" —
    // as a null dereference on the missing row — so a bare `refused: true`
    // could not tell a controlled fail-closed refusal from an accidental crash.
    // A crash is not a contract behaviour: it carries no operator-facing reason
    // and its position relative to other writes is incidental rather than
    // guaranteed. So the message is asserted too.
    expect(threw).toContain("no recorded payment movement");
   ...
   *
   * ⚠️ FIXTURE CORRECTED WHEN THE IMPLEMENTATION LANDED, AND THE REASON MATTERS.
   *
   * The design-phase version constructed this on a NORMALLY CLEARED cheque,
   * because at the time `main` could not produce a safe-harbour row at all and
   * that was the closest available shape. Under the finished design that
   * construction asserts something false: a cheque clearing against a PROVEN
   * target MUST allocate, so demanding `hasAllocationLink: false` there would
   * forbid correct behaviour.
   *
   * "Retained row" in F10 means the SAFE-HARBOUR row specifically. That row is
   * now constructible, so the cell is built on one — the same target drift as
   * A3b′. This measures F10's stated subject rather than a stand-in for it.
   *
   * NOTHING IS LOST BY THE CHANGE: the happy-path property this used to sit on
   * (allocation present when the target is proven) is asserted below in the same
   * cell, and independently by A10 and L1. The cell got stricter, not looser.
   *
   * BASE: depends on SCRUM-109 for the row's twin, and on `sales.update` being
   * able to cancel — see X6a's note.
   */
  test("R10a · the retained safe-harbour row is the exact driver-owned row, with no AR allocation", async () => {
    const { t, orgId, customerId, asUser, asApprover } = await setup();
    const { saleId } = await completedSale(t, asUser, orgId, customerId, "1HGCM82633A000034");
    const rowId = await saleLinkedRow(asUser, orgId, customerId, saleId);
    ...
    // DRIFT: the target invoice dies between registration and clearing, so the
    // clearing must reach the safe harbour and retain a zero-applied row.
    await asApprover.mutation(api.sales.update, { orgId, saleId, status: "CANCELLED" });
    await asUser.mutation(api.collections.clearCheque, { orgId, chequeId });

    // The control: a cheque against a PROVEN target still allocates, so this
    // cell cannot be satisfied by an implementation that simply stopped
    // allocating on the cheque path.
    const provenRowId = await standaloneRow(asUser, orgId, customerId);
    const provenChequeId = await asUser.mutation(api.collections.registerCheque, {
      orgId, receivableId: provenRowId, customerId,
      bank: "ABC", chequeNumber: "CQ-R10A-OK", chequeDate: Date.now(), amount: PAY,
    });
    await asUser.mutation(api.collections.clearCheque, { orgId, chequeId: provenChequeId });
    const provenHasAllocation = await t.run(async (ctx: any) => {
      const row = await ctx.db
        .query("collectionPayments")
        .withIndex("by_cheque", (q: any) => q.eq("chequeId", provenChequeId))
        .first();
      return Boolean(row?.paymentAllocationId);
    });
    expect(provenHasAllocation).toBe(true);
   ...
   *
   * ⚠️⚠️ IT IS NOW GREEN, AND SCRUM-123 IS **NOT** FIXED. READ THIS BEFORE
   * CONCLUDING OTHERWISE.
   *
   * The correlated row's money now goes to the SALE INVOICE (R2), so the
   * pre-existing guard in `cancelSaleReceivableIfSafe` finally sees allocations
   * where it was already looking, and refuses. Correct routing made an existing
   * guard effective; no cancellation logic was touched by this branch.
   *
   * **This is the exact shape that misled me on #241** — there, a patch's own
   * routing produced a refusal and I reported the path as "safe by fail-closed
   * refusal". The property belonged to the patch, not the path. The same
   * caveat applies here in the same direction, so it is written down rather
   * than left to be rediscovered: SCRUM-123's defect is that cancellation does
   * not refuse when the money sits somewhere the guard does not look. Route the
   * money elsewhere again and it returns.
   */
