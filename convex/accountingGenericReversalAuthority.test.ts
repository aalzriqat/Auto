 ...
 *
 *   COLLECTION_PAYMENT / paymentIntents     Payment Links, ruled OUT of the
 *                                           receipt occurrence by SCRUM-237
 ...
 * occurrence, a Payment Links receipt is a different source authority, and a
 * refund is not a receipt.
 *
 * ⚠️ GR7 WAS RE-VEHICLED DURING RC INTEGRATION — SCRUM-313.
 *
 * It rode `COLLECTION_PAYMENT / transactions` (`accountingMigration.ts:385-386`)
 * because that was a live producer at this artifact's branch point, and the
 * paragraph above said so. SCRUM-234 has since retired `transactions` as a
 * source AUTHORITY at the posting boundary, and that landed on protected main
 * (`42e8711ec`) AFTER the branch point, so the artifact could not have adapted:
 * ported verbatim, GR7 cannot even SEED its row, failing with SCRUM-234's
 * refusal before it reaches the reversal it is about.
 *
 * The property is untouched and is what matters — the `sourceType` column of the
 * refusal predicate is load-bearing — so it is re-pointed at another real
 * non-certified source family rather than deleted. `paymentIntents` is a
 * deliberate choice over an invented name: SCRUM-237 ruled Payment Links OUT of
 * the direct-collection receipt occurrence explicitly, so a
 * `COLLECTION_PAYMENT / paymentIntents` row is exactly what §6 needs — a real
 * event that matches the certified `eventType` and not the certified
 * `sourceType`. Nothing about the assertion changed.
 */
describe("SCRUM-254 §6 — both columns of the certified tuple discriminate", () => {
  test("GR7 — COLLECTION_PAYMENT sourced from `paymentIntents` is NOT the certified tuple and still reverses", async () => {
    ...
    const posted = await asOperator.mutation(internal.accountingLedger.post, {
      ...
      // The one column that differs from the certified occurrence. Payment Links
      // are a real source family that SCRUM-237 ruled OUT of the receipt
      // occurrence — see the block comment for why this replaced `transactions`.
      ...
      occurredAt: now,
      currency: "USD",
      idempotencyKey: "pi_gr7_post",
      payload: receiptPayload("pi_gr7", customerId.toString(), 3000),
    });
    ...
    const result = await asOperator.mutation(internal.accountingLedger.reverse, {
      ...
      originalEventId: posted.eventId!,
      reversalDate: now,
      reason: "a Payment Links receipt is outside this ticket's boundary",
      idempotencyKey: "pi_gr7_reversal",
    });
