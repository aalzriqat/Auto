...
describe("a taxed owned sale reaches both books with the same amount", () => {
  test("the GL receivable, the subledger document and the invoice all agree", async () => {
    ...
    const saleId = await asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-139-60",
      orgId,
      vehicleId,
      customerId,
  ...
  test("a tax larger than the price is refused, and nothing reaches either book", async () => {
    ...
    await expect(
      asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-192-41",
        orgId,
        vehicleId,
        customerId,
        salespersonId: userId,
  ...
  test("a tax equal to the price is still allowed — the bound is a ceiling, not a ban", async () => {
    ...
    await asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-219-45",
      orgId,
      vehicleId,
      customerId,
      salespersonId: userId,
  ...
  test("a PENDING draft may carry no agreed price, and stays editable", async () => {
    ...
    await expect(
      s.asUser.mutation(api.sales.completeDraft, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-259-50", orgId: s.orgId, saleId: draftId })
  ...
  test("a price that rounds to nothing is refused BEFORE anything financial is written", async () => {
    ...
    await expect(
      s.asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-275-43",
        orgId: s.orgId,
        vehicleId: s.vehicleId,
        customerId: s.customerId,
        salespersonId: s.userId,
  ...
  test("the smallest representable price gets past the invariant — it is not 'refuse small numbers'", async () => {
    ...
    await s.asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-303-47",
      orgId: s.orgId,
      vehicleId: s.vehicleId,
      customerId: s.customerId,
      salespersonId: s.userId,
  ...
  test("the invariant refuses BEFORE the outbox, even when accounting would be deferred", async () => {
    ...
    await expect(
      s.asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-329-43",
        orgId: s.orgId,
        vehicleId: s.vehicleId,
        customerId: s.customerId,
        salespersonId: s.userId,
  ...
  test("a zero-price sale is refused whether or not it carries add-ons", async () => {
    ...
    await expect(
      withFees.asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-366-50",
    ...
    await expect(
      bare.asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-386-46",
        orgId: bare.orgId,
  ...
  test("a sale queued against a closed period carries the convention marker and drains to the same number", async () => {
    ...
    const priorYear = new Date().getUTCFullYear() - 1;
    const saleId = await asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-416-60",
      orgId,
      vehicleId,
      customerId,
      salespersonId: userId,
  ...
  test("an untaxed sale is unchanged in both books", async () => {
    ...
    const saleId = await asUser.mutation(api.sales.create, { idempotencyKey: "t-ownedSaleTaxEndToEnd.test-477-60",
      orgId,
      vehicleId,
      customerId,
      salespersonId: userId,
