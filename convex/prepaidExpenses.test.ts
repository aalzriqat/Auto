...
describe("prepaid expense — initial posting", () => {
  test("a prepaid expense debits Prepaid Expenses (asset), not an expense account, and opens a schedule", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-138-67",
  ...
  test("VAT is split out immediately; only the net amount capitalizes to the asset and amortizes", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-160-67",
...
describe("prepaid expense — monthly amortization", () => {
  test("one month releases a share from the asset to the expense account, idempotently", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-183-67",
  ...
  test("recognition inside the start month is never dated before the debit it releases", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-237-67",
  ...
  test("strict month ordering: a month at/before the last recognized one is rejected", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-270-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
  ...
  test("runs to full term, then stops without over-recognizing; asset nets to zero", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-285-67",
...
describe("prepaid expense — reversal unwinds the whole lifecycle", () => {
  test("reversing after partial amortization nets every account to zero and cancels the schedule", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-309-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
  ...
  test("reversing after a partial refund correction also unwinds the refund — no orphaned cash or negative prepaid balance", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-329-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-337-65",
  ...
  test("reversing after an accelerated write-off correction also unwinds the write-off — no orphaned expense or negative prepaid balance", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-355-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-364-65",
  ...
  test("reversing after a combined VAT refund + write-off unwinds both corrections", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-377-67",
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-384-65",
      orgId, scheduleId: schedule!._id, refundMinor: 200_000, refundTaxMinor: 40_000,
      refundPaymentMethod: "CASH", reason: "Partial refund",
    });
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-388-65",
...
describe("prepaid expense — operational report matches the GL", () => {
  test("the operational P&L recognizes exactly what the GL amortized for the month", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-407-49",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
  ...
  test("Fix B1 — with VAT, the report recognizes the NET monthly share (from the schedule), not gross", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-426-49",
      orgId, title: "Prepaid rent w/ VAT", amount: 1200, taxAmount: 200, date: Date.UTC(2026, 0, 1),
      category: "RENT", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 10,
    });
    const schedule = await t.run((ctx) => ctx.db.query("prepaidExpenseSchedules").first());
  ...
  test("a prepaid schedule that started before the reporting window still amortizes into it (schedule-driven, no lookback cap)", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-444-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
...
describe("Phase 1 — report derives from posted/parked recognition events, never a curve recompute", () => {
  test("a write-off's accelerated expense shows up in its own correction month, never restating already-posted months", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-472-49",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await t.run((ctx) => ctx.db.query("prepaidExpenseSchedules").first());
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-487-65",
  ...
  test("a refund never appears in the P&L (cash vs asset, not an expense), and history/future months are unaffected", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-521-49",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await t.run((ctx) => ctx.db.query("prepaidExpenseSchedules").first());
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-530-65",
  ...
  test("a correction that consumes the full remainder cancels the schedule, but prior months still report with no expense-doc double count", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-557-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-564-65",
  ...
  test("a month parked in the outbox behind a closed period still reports in its own month", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-587-49",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await t.run((ctx) => ctx.db.query("prepaidExpenseSchedules").first());
...
describe("prepaid expenses reconciliation", () => {
  test("GL Prepaid asset equals the sum of ACTIVE schedules' remaining", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-623-49",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await t.run((ctx) => ctx.db.query("prepaidExpenseSchedules").first());
...
describe("Fix B2/B9 — a period with prepaid amortization due but unrecognized cannot close", () => {
  test("a schedule the cron never advanced blocks the close, and recognizing it unblocks", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-645-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
...
describe("Fix B3 — catch-up recognizes each missed month in its own month, never lumped into the present", () => {
  test("a schedule behind by several months posts one dated event per month", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-674-49",
...
describe("correctSchedule — a partial correction stays consistent with future recognition", () => {
  test("a write-off that doesn't end the schedule re-bases future months off the remaining balance, not the original curve", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-708-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-723-65",
      orgId, scheduleId: schedule!._id, writeOffMinor: 300_000, reason: "Early cancellation, non-refundable portion",
    });
  ...
  test("shortening the term doesn't produce a false shortfall once caught up under the new term", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-755-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-771-65",
  ...
  test("a term correction that leaves an unrecognized remainder with no future month is rejected", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-784-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    // recognize it.
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-799-61",
...
describe("Phase 2 — correctSchedule term cap and idempotency", () => {
  test("rejects a corrected term above the 600-month cap (matches expense creation's own cap)", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-809-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-815-61",
  ...
  test("the same idempotency key replayed twice applies the correction exactly once", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-823-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
  ...
  test("a different idempotency key applies a second, independent correction", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-859-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
...
describe("Phase 4 — VAT-aware refunds", () => {
  test("a refund with a VAT portion posts a balanced 3-line journal: cash debit gross, Prepaid Expenses credit net, VAT_RECEIVABLE credit tax", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-883-49",
      orgId, title: "Prepaid rent w/ VAT", amount: 1200, taxAmount: 200, date: Date.UTC(2026, 0, 1),
      category: "RENT", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 10,
    });
    const schedule = await t.run((ctx) => ctx.db.query("prepaidExpenseSchedules").first());
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-894-65",
  ...
  test("the VAT refund cap is enforced against the expense's original taxAmount, net of prior VAT refunds", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-912-49",
      orgId, title: "Prepaid rent w/ VAT", amount: 1200, taxAmount: 200, date: Date.UTC(2026, 0, 1),
      category: "RENT", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 10,
    });
    const schedule = await t.run((ctx) => ctx.db.query("prepaidExpenseSchedules").first());
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-920-65",
    ...
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-927-61",
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-934-65",
  ...
  test("a VAT refund without an accompanying net refund is rejected", async () => {
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-947-49",
      orgId, title: "Prepaid rent w/ VAT", amount: 1200, taxAmount: 200, date: Date.UTC(2026, 0, 1),
      category: "RENT", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 10,
    });
    const schedule = await t.run((ctx) => ctx.db.query("prepaidExpenseSchedules").first());
    ...
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-955-61",
  ...
  test("a refund with no VAT posts the same two-line journal as before (byte-identical zero-tax path)", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-963-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-970-65",
...
describe("Phase 6 — maker-checker write-off approval", () => {
  test("a non-owner's write-off creates a PENDING request instead of applying, and the schedule is untouched", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-982-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const result = await asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-990-85",
  ...
  test("the owner's own write-off still applies directly, without a request", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1015-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const result = await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1022-80",
  ...
  test("the maker cannot approve their own request", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1036-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const result = await asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1044-85",
  ...
  test("a different finance manager (or the owner) can approve, applying the correction and posting the GL", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1059-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const result = await asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1067-85",
  ...
  test("a request whose remainder shrank while pending is rejected cleanly at approval time", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1093-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const result = await asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1102-85",
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1110-65",
  ...
  test("rejecting a request applies no schedule change", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1127-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const result = await asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1135-85",
      orgId, scheduleId: schedule!._id, writeOffMinor: 300_000, reason: "Needs approval",
    });
    const requestId = result.requestId!;
  ...
  test("listPendingCorrectionRequests only returns PENDING requests for the org, enriched with schedule/expense details", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1151-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1159-70",
      orgId, scheduleId: schedule!._id, writeOffMinor: 300_000, reason: "Needs approval",
    });
...
describe("listSchedules — pending/failed totals only count amortization, not corrections", () => {
  test("a pending refund event is not counted as pending amortization", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1210-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
  ...
  test("a failed write-off event is counted as a failed correction, not failed amortization", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1236-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
...
describe("Phase 3 — runAmortizationNow isolates per-schedule failures and reports blocked schedules", () => {
  test("one schedule's posting failure doesn't roll back its own progress or block the others", async () => {
    ...
    const goodExpenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1262-71",
      orgId, title: "Good Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const goodSchedule = await scheduleForExpense(t, goodExpenseId);

    const badExpenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1268-70",
...
describe("Phase 3 — redriveScheduleEvents", () => {
  test("redrives only the target schedule's own queued/dead-lettered rows, resetting a dead-lettered row's attempts", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1332-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const otherExpenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1340-72",
  ...
  test("nothing queued for a clean schedule redrives zero rows", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1401-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
...
describe("Fix #4 — chart self-heal never duplicates or hijacks a code", () => {
  test("an incompatible custom account on the reserved code blocks the posting with a clear error", async () => {
    ...
    await expect(
      asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1434-45",
        orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
        category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
      })
  ...
  test("a compatible unmapped account on the reserved code parks an adoption request instead of silently adopting (Fix #10)", async () => {
    ...
    await expect(
      asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1461-45",
        orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
        category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
      })
    ...
    await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1477-49",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
...
describe("prepaid corrections — refuse to post against an unbooked asset", () => {
  ...
  test("rejects a write-off while the source expense is still queued", async () => {
    ...
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1598-61",
  ...
  test("rejects a refund while the source expense is still queued", async () => {
    ...
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1612-61",
  ...
  test("rejects a VAT-bearing refund while the source expense is still queued", async () => {
    ...
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1628-61",
  ...
  test("writes no correction row and leaves the schedule untouched when refused", async () => {
    ...
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1643-61",
  ...
  test("refuses at submission, so a request is never even queued for approval", async () => {
    ...
    await expect(
      asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1664-66",
        orgId, scheduleId, writeOffMinor: 300_000, reason: "Early cancellation",
      })
    ).rejects.toThrow(NOT_POSTED);
  ...
  test("re-checks at approval: a request approved after the source un-posts is refused", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1679-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const submitted = await asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1687-88",
      orgId, scheduleId: schedule!._id, writeOffMinor: 300_000, reason: "Early cancellation",
    });
  ...
  test("allows a term-only correction while the source expense is queued", async () => {
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1722-65",
  ...
  test("allows the correction once the source expense has actually posted", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1733-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1740-65",
      orgId, scheduleId: schedule!._id, writeOffMinor: 300_000, reason: "Early cancellation",
    });
...
describe("prepaid corrections — refuse to post ahead of the original entry", () => {
  ...
  async function seedFutureDatedPrepaid(t: T, orgId: Id<"organizations">, asOwner: Awaited<ReturnType<typeof seedDealer>>["asOwner"]) {
    ...
    try {
      const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1768-69",
  ...
  test("rejects a refund dated before the original entry", async () => {
    ...
    try {
      await expect(
        asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1804-63",
          orgId, scheduleId, refundMinor: 300_000, refundPaymentMethod: "BANK_TRANSFER",
  ...
  test("rejects a write-off dated before the original entry", async () => {
    ...
    try {
      await expect(
        asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1824-63",
  ...
  test("allows a term-only correction on a future-dated expense", async () => {
    ...
    const clock = vi.spyOn(Date, "now").mockReturnValue(JUL);
    try {
      await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1843-67",
        orgId, scheduleId, newTermMonths: 6, reason: "Corrected coverage period",
      });
    } finally {
      clock.mockRestore();
  ...
  test("allows the refund once the original entry's date has been reached", async () => {
    ...
    try {
      await asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1861-67",
        orgId, scheduleId, refundMinor: 300_000, refundPaymentMethod: "BANK_TRANSFER",
  ...
  test("re-checks ordering at approval, not just at submission", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1881-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    const submitted = await asAccountant.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-1889-88",
      orgId, scheduleId: schedule!._id, writeOffMinor: 300_000, reason: "Early cancellation",
    });
    expect(submitted.status).toBe("PENDING");
...
describe("prepaid corrections — the accountant can date the correction", () => {
  async function seedPostedPrepaid(tag: string, expenseDate = Date.UTC(2026, 0, 1)) {
    ...
    const expenseId = await ctx.asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-1931-71",
...
describe("prepaid corrections — the outbox refuses to post them against an unbooked asset", () => {
  ...
  test("a healthy queued correction still drains normally", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-2266-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
...
describe("prepaid corrections — non-finite money inputs", () => {
  test("a NaN refund is rejected instead of corrupting the schedule", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-2390-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-2401-61",
        orgId,
  ...
  test("a NaN write-off cannot slip past the maker-checker approval gate", async () => {
    ...
    const expenseId = await asOwner.mutation(api.expenses.create, { idempotencyKey: "t-prepaidExpenses.test-2417-67",
      orgId, title: "Insurance", amount: 1200, date: Date.UTC(2026, 0, 1),
      category: "FEES", status: "PAID", paymentMethod: "CASH", isPrepaid: true, amortizationMonths: 12,
    });
    const schedule = await scheduleForExpense(t, expenseId);
    ...
    await expect(
      asOwner.mutation(api.prepaidExpenses.correctSchedule, { idempotencyKey: "t-prepaidExpenses.test-2424-61",
        orgId,
        scheduleId: schedule!._id,
        writeOffMinor: NaN,
