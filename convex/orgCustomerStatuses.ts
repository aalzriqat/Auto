...
async function detachStatusFromFinanceCompanies(
  ctx: MutationCtx,
  ...
  for (const company of companies) {
    if (!company.acceptedStatuses?.includes(statusId)) continue;
    const remaining = company.acceptedStatuses.filter((id) => id !== statusId);
...
export const remove = mutation({
  ...
  handler: async (
    ...
    // Deactivation and deletion share one fail-closed lender-scope path.
    const { updatedCompanies, deactivatedCompanies } =
