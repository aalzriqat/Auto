...
const HISTORICAL_ENTRIES: HistoricalChangelogEntry[] = [
  ...
  },
  {
    ...
    descriptionEn: "Open any lead and you'll see exactly what happened to it and who did it — every stage change, reassignment, and edit, including the ones made automatically by Instagram, Facebook, WhatsApp, the dealer website and the marketplace. Automated changes name what triggered them (\"Quote sent\", \"Sale recorded\") instead of just saying \"System\". The trail can't be edited or deleted, and it survives a lead being deleted and restored.",
    descriptionAr: "افتح أي عميل محتمل لترى بالضبط ما حدث له ومن قام بذلك — كل تغيير في المرحلة، وكل إعادة تعيين، وكل تعديل، بما في ذلك ما يتم تلقائياً عبر إنستغرام وفيسبوك وواتساب وموقع المعرض والسوق. كما تُوضّح التغييرات التلقائية سببها (\"تم إرسال عرض سعر\"، \"تم تسجيل بيع\") بدلاً من الاكتفاء بكلمة \"النظام\". ولا يمكن تعديل السجل أو حذفه، ويبقى محفوظاً حتى بعد حذف العميل المحتمل واستعادته.",
    publishedAt: releaseAt(2026, 7, 30, 1),
  },
  {
    type: "FEATURE",
    titleEn: "Log each follow-up as its own update",
    titleAr: "سجّل كل متابعة كتحديث مستقل",
    descriptionEn: "Salespeople can now add a dated update to a lead each time they follow up, instead of overwriting the same Notes box again and again. Every update is permanent and appears in the lead's activity trail, so \"called twice, no answer\" on Tuesday is still there after Thursday's \"wants a test drive\".",
    descriptionAr: "أصبح بإمكان مندوبي المبيعات إضافة تحديث مؤرّخ للعميل المحتمل في كل مرة يقومون فيها بالمتابعة، بدلاً من الكتابة فوق حقل الملاحظات نفسه مراراً. وكل تحديث دائم ويظهر ضمن سجل نشاط العميل المحتمل، فتبقى ملاحظة \"اتصلت مرتين بدون رد\" يوم الثلاثاء موجودة بعد ملاحظة \"يريد تجربة قيادة\" يوم الخميس.",
    publishedAt: releaseAt(2026, 7, 30, 2),
  },
  {
    type: "FEATURE",
    titleEn: "See what the customer actually asked, on the lead",
    titleAr: "اطّلع على ما سأل عنه العميل فعلياً من داخل العميل المحتمل",
    descriptionEn: "Leads created from Instagram and Facebook now show the customer's own comments and messages at the top of the lead, read-only, with a count of anything still unanswered. Previously the lead only carried the truncated first message, so everything the customer asked afterwards was invisible.",
    descriptionAr: "أصبحت العملاء المحتملون القادمون من إنستغرام وفيسبوك تعرض تعليقات ورسائل العميل نفسه في أعلى بطاقة العميل المحتمل، للقراءة فقط، مع عدد الرسائل التي لم يتم الرد عليها. في السابق كان العميل المحتمل يحمل الرسالة الأولى المختصرة فقط، فكان كل ما سأل عنه العميل بعدها غير ظاهر.",
    publishedAt: releaseAt(2026, 7, 30, 3),
  },
  {
    type: "IMPROVEMENT",
    titleEn: "Exclude a salesperson from automatic lead assignment",
    titleAr: "استثناء مندوب مبيعات من التوزيع التلقائي للعملاء المحتملين",
    descriptionEn: "Team > Members now has a per-person switch that takes someone out of the automatic round robin for leads from social, the website, WhatsApp and the marketplace — useful for a rep on leave or a manager who holds the Sales role only for reporting. Their existing leads are untouched, and a manager can still assign them a lead by hand.",
    descriptionAr: "أصبحت صفحة الفريق < الأعضاء تتضمن مفتاحاً لكل شخص يستثنيه من التوزيع التلقائي بالتناوب للعملاء المحتملين القادمين من وسائل التواصل والموقع وواتساب والسوق — وهو مفيد لمندوب في إجازة أو لمدير يحمل دور المبيعات لأغراض التقارير فقط. ولا تتأثر العملاء المحتملون الحاليون لديه، ويظل بإمكان المدير تعيين عميل محتمل له يدوياً.",
    publishedAt: releaseAt(2026, 7, 30, 4),
  },
  {
    type: "FIX",
    titleEn: "Lead dialog no longer shows a blank customer or source",
    titleAr: "لم تعد نافذة العميل المحتمل تعرض عميلاً أو مصدراً فارغاً",
    descriptionEn: "Editing a lead created from Instagram, Facebook or WhatsApp showed an empty Customer and Lead Source box, because the customer sat beyond the first page the dropdown loads and the source (\"Instagram DM\") wasn't in the configured source list. Both now display the lead's real values. The same fix covers a lead pointing at a vehicle that has since been sold.",
