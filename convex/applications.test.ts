...
describe("applications.finalizeDeal", () => {
  test("closes the quote's lead as WON and stamps quoteId/leadId on the sale", async () => {
    ...
    await asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-applications.test-110-58", orgId, applicationId });
...
describe("applications hold release and deposit resolution", () => {
  test("rejected applications expose held deposits for resolution after releasing the vehicle hold", async () => {
    ...
    const depositId = await asUser.mutation(api.deposits.create, { idempotencyKey: "t-applications.test-610-66",
      orgId,
      quoteId,
      amount: 1000,
    });
  ...
  test("cancelling a submitted application releases a same-customer reservation without a deposit", async () => {
    ...
    await asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-applications.test-753-63",
      orgId,
      applicationId,
  ...
  test("cancelling a submitted application releases a same-customer reservation deposit hold", async () => {
    ...
    await asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-applications.test-804-63",
      orgId,
      applicationId,
      reason: "Customer changed vehicles",
    });
  ...
  test("rerunning cancellation on an already-cancelled application releases stale reservations", async () => {
    ...
    await asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-applications.test-853-63",
      orgId,
      applicationId,
  ...
  test("cancellation rejects missing applications, approved apps without approval rights, and disbursed closed deals", async () => {
    ...
    await expect(
      asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-applications.test-895-59", orgId, applicationId: otherApplicationId })
    ).rejects.toThrow(/application not found/i);
    ...
    await expect(
      asLimited.mutation(api.applications.cancelApplication, { idempotencyKey: "t-applications.test-929-62",
        orgId,
    ...
    await expect(
      asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-applications.test-942-59",
        orgId,
        applicationId: approvedApplicationId,
      })
  ...
  test("cancelling a closed deal with a commission cancels the underlying sale", async () => {
    ...
    await asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-applications.test-964-63",
      orgId,
      applicationId,
...
async function setupFinalizedFinancedDeal() {
  ...
  const depositId = await asUser.mutation(api.deposits.create, { idempotencyKey: "t-applications.test-1077-64",
    orgId,
    quoteId,
    amount: 3000,
  });
  ...
  await asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-applications.test-1107-56", orgId, applicationId });
...
describe("applications finance-company canonical receivable", () => {
  test("finalizeDeal opens a FINANCE_COMPANY receivable and confirmDisbursement settles it by allocation", async () => {
    ...
    await asUser.mutation(api.applications.confirmDisbursement, { idempotencyKey: "t-applications.test-1170-65",
      orgId,
      applicationId,
  ...
  test("confirmDisbursement rejects an amount that is not what the company owes", async () => {
    ...
    await expect(
      asUser.mutation(api.applications.confirmDisbursement, { idempotencyKey: "t-applications.test-1205-61",
        orgId,
        applicationId,
  ...
  test("voiding a finalized (undisbursed) deal cancels the finance-company receivable", async () => {
    ...
    await asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-applications.test-1221-63",
      orgId,
      applicationId,
...
describe("applications logs, expected payment, and finalization guards", () => {
  ...
  test("registerExpectedPayment requires cheque details and finalization requires handover and payment metadata", async () => {
    ...
    await expect(
      asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-applications.test-1307-54", orgId, applicationId })
    ).rejects.toThrow(/register the vehicle handover/i);

    await registerHandover(asUser, api, orgId, applicationId);
    await expect(
      asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-applications.test-1312-54", orgId, applicationId })
  ...
  test("finalizeDeal rejects quote mismatches and returns an existing closed sale idempotently", async () => {
    ...
    await expect(asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-applications.test-1332-65", orgId, applicationId })).resolves.toBe(closedSaleId);
    ...
    await expect(
      mismatched.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-applications.test-1374-65",
        orgId: mismatched.orgId,
        applicationId: applicationIdToMismatch,
      })
  ...
  test("finalizeDeal rejects finance company mismatch between application and quote", async () => {
    ...
    await expect(
      asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-applications.test-1426-54", orgId, applicationId })
...
async function setupFinalizedFinancedDealWithCheque() {
  ...
  await classifyFinancedDeal({ orgId, applicationId, asUser }, 20000);
  await asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-applications.test-1478-56", orgId, applicationId });
...
describe("applications.confirmDisbursement cheque linking", () => {
  test("confirmDisbursement transitions the linked postDatedCheques row to CLEARED", async () => {
    ...
    await asUser.mutation(api.applications.confirmDisbursement, { idempotencyKey: "t-applications.test-1498-65",
      orgId,
      applicationId,
  ...
  test("confirmDisbursement throws if the linked cheque was already returned", async () => {
    ...
    await expect(
      asUser.mutation(api.applications.confirmDisbursement, { idempotencyKey: "t-applications.test-1520-61",
        orgId,
        applicationId,
        disbursedAmountMinor: 20_000_000,
      })
  ...
  test("clearCheque refuses to clear a cheque that belongs to a finance application", async () => {
    ...
    await expect(
      asUser.mutation(api.collections.clearCheque, { idempotencyKey: "t-applications.test-1533-52", orgId, chequeId: cheque!._id })
  ...
  test("confirmDisbursement resolves the replacement cheque after the original was replaced", async () => {
    ...
    await asUser.mutation(api.applications.confirmDisbursement, { idempotencyKey: "t-applications.test-1550-65",
      orgId,
      applicationId,
      disbursedAmountMinor: 20_000_000,
    });
  ...
  test("confirmDisbursement treats a soft-deleted linked cheque as not found", async () => {
    ...
    await expect(
      asUser.mutation(api.applications.confirmDisbursement, { idempotencyKey: "t-applications.test-1568-61",
        orgId,
        applicationId,
        disbursedAmountMinor: 20_000_000,
      })
