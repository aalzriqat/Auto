...
export async function planReceiptRevocation(
  ...
  // Newest first, mirroring the order the applications were created in.
  //
  // ⚠️ CORRECTED DURING RC INTEGRATION (SCRUM-313) — THE ORIGINAL JUSTIFICATION
  // OVERCLAIMED. It said an oldest-first unwind "would restore a position it is
  // about to draw down again in the same transaction". A mutation battery
  // reversed the comparator and NOTHING went red, so that claim was checked
  // against the code rather than believed: no step of the unwind draws the
  // position down. `revokeReceiptApplicationOccurrence` never touches the
  // position at all, and `retireReceiptRevocationState` zeroes it exactly once
  // at the end. Each application reverses its OWN allocation and reopens its OWN
  // receivable by its OWN persisted amount, so the end state is genuinely
  // order-independent.
  //
  // The order is kept because a deterministic, explicable order is worth having
  // in a money unwind and because it matches creation order — NOT because
  // correctness rests on it. Stating a safety property the code does not
  // actually depend on is how a later change gets talked out of questioning it.
  //
  // ⚠️ This is the ordering of the PLAN. It is not a claim about journal-entry
  // ordering in the ledger, which nothing here promises.
