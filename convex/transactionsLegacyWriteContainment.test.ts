...
describe("SCRUM-53 — legacy transactions write doors are closed server-side", () => {
  ...
  test("a migrated, GL-represented row cannot be edited or deleted, and the books do not move", async () => {
    ...
    const id = await insertLegacyRow(s);
    // This is what makes the row GL-represented: the migration posts it and
    // records an accountingEvent with sourceType "transactions".
    await s.asOwner.mutation(api.accountingMigration.migrateUnpostedTransactions, {
      orgId: s.orgId,
      dryRun: false,
    });
