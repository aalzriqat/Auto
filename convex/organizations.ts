...
export async function createOrganizationWithDefaultRoles(
...
): Promise<{ orgId: Id<"organizations">; roleIdsByName: Record<string, Id<"roles">> }> {
  ...
  const roleIdsByName: Record<string, Id<"roles">> = {};
  ...
  for (const template of DEFAULT_ROLE_TEMPLATES) {
    ...
    roleIdsByName[template.name] = roleId;
