...
async function completedSale(d: Dealer, saleDate = Date.now(), salePrice = 15000) {
  return await d.asAdmin.mutation(api.sales.create, { idempotencyKey: "t-commissionAccrualTiming.test-240-53",
    orgId: d.orgId,
...
describe("a MANUAL commission is recognized when it becomes measurable", () => {
  ...
  test("an amount entered on a draft accrues when the sale completes, not before", async () => {
    ...
    await d.asAdmin.mutation(api.sales.completeDraft, { idempotencyKey: "t-commissionAccrualTiming.test-310-55", orgId: d.orgId, saleId });
...
describe("settlement clears exactly what was recognized", () => {
  test("paying a corrected commission directly nets Commission Payable to zero", async () => {
    ...
    await d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-539-60",
      orgId: d.orgId,
      saleId,
      paymentMethod: "CASH",
    });
...
describe("the reconciliation is point-in-time on BOTH sides", () => {
  test("a commission accrued in the period and paid after it still reconciles", async () => {
    ...
    await d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-705-60",
      orgId: d.orgId,
      saleId,
      paymentMethod: "CASH",
    });
...
describe("the backlog backfill", () => {
  ...
  test("skips cancelled, unpaid-but-zero, and already-paid commissions", async () => {
    ...
    await d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-803-60",
      orgId: d.orgId,
      saleId: paidSale,
      paymentMethod: "CASH",
    });
...
describe("no commission entry may overtake the entries it depends on", () => {
  ...
  test("a direct payment refuses when the sale's period cannot take the entry", async () => {
    ...
    await expect(
      d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-896-56",
        orgId: d.orgId,
        saleId,
        paymentMethod: "CASH",
      })
...
describe("the outbox will not drain a settlement ahead of its own accrual", () => {
  ...
  async function queuedAccrualAndPayment(suffix: string, opts: { pay?: boolean } = {}) {
    ...
    const saleId = await asAdmin.mutation(api.sales.create, { idempotencyKey: "t-commissionAccrualTiming.test-1056-61",
    ...
    if (pay) {
      await asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-1065-60",
...
describe("commission entries are dated by one rule everywhere", () => {
  test("an org with no chart queues its accrual at the SALE date, not today", async () => {
    ...
    const saleId = await asAdmin.mutation(api.sales.create, { idempotencyKey: "t-commissionAccrualTiming.test-1242-61",
      orgId, vehicleId, customerId, salespersonId: userId,
...
describe("round-3 review fixes", () => {
  ...
  test("settlement refuses when the sale's amount no longer matches what was recognized", async () => {
    ...
    await expect(
      d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-1423-56",
        orgId: d.orgId,
        saleId,
        paymentMethod: "CASH",
      })
...
describe("a closed period blocks a commission only when something must post into it", () => {
  test("a commission accrued before its month closed can still be paid afterwards", async () => {
    ...
    await d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-1988-60",
      orgId: d.orgId,
      saleId,
      paymentMethod: "CASH",
    });
  ...
  test("a commission that never accrued is still refused once its month is closed", async () => {
    ...
    await expect(
      d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-2017-56",
        orgId: d.orgId,
        saleId,
        paymentMethod: "CASH",
      })
  ...
  test("payroll approval SKIPS a stranded sale instead of failing the whole run", async () => {
    ...
    const liveSaleId = await d.asAdmin.mutation(api.sales.create, { idempotencyKey: "t-commissionAccrualTiming.test-2073-67",
      orgId: d.orgId,
...
describe("the period that must accept a payment is today's, not the sale's", () => {
  test("paying a commission is refused when TODAY's period is closed", async () => {
    ...
    await expect(
      d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-2180-56",
        orgId: d.orgId,
        saleId,
        paymentMethod: "CASH",
      })
...
describe("the close checklist names the remedy that fits each unrecognized commission", () => {
  test("a closed-period commission and a no-period commission are reported apart", async () => {
    ...
    const noPeriodSaleId = await d.asAdmin.mutation(api.sales.create, { idempotencyKey: "t-commissionAccrualTiming.test-2439-71",
      orgId: d.orgId,
...
describe("approval reports the commissions it left out", () => {
  test("skippedCommissionSaleIds names the stranded sale", async () => {
    ...
    const liveSaleId = await d.asAdmin.mutation(api.sales.create, { idempotencyKey: "t-commissionAccrualTiming.test-2490-67",
...
describe("a posted entry with an unreadable amount fails closed", () => {
  test("settlement is refused rather than compared against a partial total", async () => {
    ...
    await expect(
      d.asAdmin.mutation(api.sales.markCommissionPaid, { idempotencyKey: "t-commissionAccrualTiming.test-2647-56",
        orgId: d.orgId,
        saleId,
        paymentMethod: "CASH",
      })
