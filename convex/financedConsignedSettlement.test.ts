...
async function runDeal(
  ...
  if (opts.deposit) {
    ...
    await taker.mutation(api.deposits.create, { idempotencyKey: "t-financedConsignedSettlement.test-219-47",
      orgId: s.orgId,
      quoteId,
      amount: opts.deposit,
    });
  ...
  if (opts.depositAfterRoute) {
    await s.asUser.mutation(api.deposits.create, { idempotencyKey: "t-financedConsignedSettlement.test-256-50",
      orgId: s.orgId,
      quoteId,
  ...
  if (throughRouteNeedsEconomics) {
    ...
    const feeId = await s.asUser.mutation(api.financeDealCosts.recordDealFee, { idempotencyKey: "t-financedConsignedSettlement.test-333-79",
      orgId: s.orgId,
      applicationId,
  ...
  const saleId = await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-385-73",
    orgId: s.orgId,
    applicationId,
...
describe("confirming the money, on each route", () => {
  test("confirmDisbursement is refused: that money never reaches the dealership's account", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.confirmDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-563-63",
  ...
  test("the supplier confirmation records the advice and posts no journal at all", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-574-75",
      orgId: s.orgId,
      applicationId,
  ...
  test("it does not settle the supplier's claim — he has been paid, the dealership has not", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-601-75",
      orgId: s.orgId, applicationId, disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
  ...
  test("the supplier confirmation is refused on a deal that settles through the dealership", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-628-71",
        orgId: s.orgId, applicationId, disbursedAmountMinor: VEHICLE_PRICE * SCALE,
      })
...
describe("THROUGH_DEALERSHIP is unchanged by any of this", () => {
  ...
  test("a new consigned financed deal cannot be finalized without choosing a route", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-662-56", orgId: s.orgId, applicationId })
...
describe("cancelling after the finance company has paid the supplier", () => {
  test("is refused, the way it is once the dealership itself has been paid", async () => {
    ...
    const { applicationId } = await runDeal(s, { route: "DIRECT_TO_SUPPLIER" });
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-808-75",
      orgId: s.orgId, applicationId, disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
    ...
    await expect(
      s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-818-61",
...
describe("cancelling by the other door, after the supplier was paid", () => {
  test("sales.update cannot cancel a deal the finance company has already paid the supplier for", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-834-75",
      orgId: s.orgId, applicationId, disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
...
describe("the settlement advice is recorded as stated", () => {
  test("an amount that differs from the customer's financing principal is stored verbatim", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-876-75",
  ...
  test("a nonsensical disbursement date is refused rather than stored", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-898-71",
...
describe("cancelling a direct-settled financed deal", () => {
  test("cancels the supplier's claim and reverses nothing that was never posted", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-912-65",
      orgId: s.orgId, applicationId, reason: "Customer withdrew",
    });
...
describe("an external financier the deal does not name with a companyId", () => {
  ...
  test("a manual finance company can settle direct to the supplier", async () => {
    ...
    const result = await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-962-90",
      orgId: s.orgId, applicationId, disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
...
describe("a legacy deal that has a finance company but no recorded mode", () => {
  ...
  test("can still take the direct route", async () => {
    ...
    const result = await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1043-90",
      orgId: s.orgId, applicationId, disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
    expect(result.disbursedAmountMinor).toBe(VEHICLE_PRICE * SCALE);
  });
...
describe("the deal cockpit query", () => {
  ...
  test("the headline figure never travels without its classification", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1245-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
  ...
  test("the direct-route headline agrees with the canonical dealer economics when the advice is not the sale price", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-1339-60", orgId: s.orgId, applicationId });
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1340-75",
      orgId: s.orgId,
      applicationId,
...
describe("the deal cockpit, under the conditions that broke it", () => {
  ...
  test("collecting from the supplier does not change the deal's profit", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1472-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
    ...
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-1483-68",
      orgId: s.orgId,
  ...
  test("only expenses the dealership actually bore reduce its profit", async () => {
    ...
    // The headline now derives from the RECORDED advice rather than from the
    // approved amount, so the advice has to exist for there to be a figure.
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1510-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
  ...
  test("an unreconciled expense keeps the figure estimated", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1675-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
...
describe("a direct-settled deal that is genuinely finished", () => {
  test("reaches its final stage and stops calling the figure an estimate", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1708-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
    const claim = (await supplierClaimsOf(s)).find((row) => row.status !== "CANCELLED")!;
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-1714-68",
      orgId: s.orgId,
      receivableId: claim._id,
      amount: MARGIN,
    });
...
describe("proving a deal is settled, rather than assuming it", () => {
  ...
  test("a partial supplier disbursement does not settle the deal, even though it is recorded", async () => {
    ...
    const result = await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1792-90",
      orgId: s.orgId,
      applicationId,
  ...
  test("an advice smaller than the margin cannot move the headline, and is flagged", async () => {
    ...
    const result = await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1834-90",
      orgId: s.orgId,
      applicationId,
  ...
  test("a zero-margin direct deal can finish, because there is nothing to collect", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1893-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
  ...
  test("a fully paid direct deal still completes", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-1919-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
    const claim = (await supplierClaimsOf(s)).find((row) => row.status !== "CANCELLED")!;
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-1925-68",
      orgId: s.orgId,
      receivableId: claim._id,
      amount: MARGIN,
    });
...
describe("what the supplier-receipt mutation accepts", () => {
  ...
  test("a receipt dated in the future is refused", async () => {
    ...
    await expect(
      s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-1957-64",
        orgId: s.orgId,
        receivableId: claim._id,
        amount: 1_000,
  ...
  test("a receipt date that is not a real instant is refused", async () => {
    ...
    await expect(
      s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-1969-64",
        orgId: s.orgId,
        receivableId: claim._id,
        amount: 1_000,
  ...
  test("an amount finer than the currency can represent is refused", async () => {
    ...
    await expect(
      s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-1987-64",
        orgId: s.orgId,
        receivableId: claim._id,
        amount: 0.0005,
      })
...
describe("the party rows and the stage rail agree", () => {
  test("a zero-margin direct deal shows nothing outstanding, not an unknown", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-2097-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
...
describe("settlement derived from sale-time facts, in integer minor units", () => {
  ...
  async function directDeal(tag: string) {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-2162-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
  ...
  test("a claim paid in instalments that do not sum exactly is still fully settled", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-2190-70",
        orgId: s.orgId,
        receivableId: claim._id,
        amount: 1.48,
      });
  ...
  test("editing the vehicle cost after the sale does not disturb a settled deal", async () => {
    ...
    const claim = (await supplierClaimsOf(s)).find((row) => row.status !== "CANCELLED")!;
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-2274-68",
      orgId: s.orgId,
      receivableId: claim._id,
      amount: MARGIN,
    });
  ...
  test("the headline nets the dealership contribution, and shows it as its own line", async () => {
    ...
    const claim = (await supplierClaimsOf(s)).find((row) => row.status !== "CANCELLED")!;
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-2324-68",
      orgId: s.orgId,
      receivableId: claim._id,
      amount: MARGIN,
    });
  ...
  test("an unrecorded contribution withholds the headline rather than assuming zero", async () => {
    ...
    const claim = (await supplierClaimsOf(s)).find((row) => row.status !== "CANCELLED")!;
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-2356-68",
      orgId: s.orgId,
      receivableId: claim._id,
      amount: MARGIN,
    });
  ...
  test("a vehicle re-sold after a cancellation is judged on the live sale only", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-2424-75",
      orgId: s.orgId,
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-2451-75",
      orgId: s.orgId,
  ...
  test("a recorded zero margin still lets the deal complete", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-2540-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
  ...
  test("a payable paid in instalments that do not sum exactly is settled on both surfaces", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await s.asApprover.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-financedConsignedSettlement.test-2620-78",
        orgId: s.orgId,
  ...
  test("a payment finer than the currency can represent is refused", async () => {
    ...
    await expect(
      s.asApprover.mutation(api.sourcingPayables.recordPartialPayment, { idempotencyKey: "t-financedConsignedSettlement.test-2658-72",
        orgId: s.orgId,
        payableId: payable._id,
        amount: 0.0005,
      })
...
describe("the supplier is never made debtor for money that did not reach him", () => {
  ...
  test("a supplier paid exactly his entitlement owes nothing, and no claim is opened", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-2865-60", orgId: s.orgId, applicationId });
  ...
  test("a supplier paid 3,000 above his entitlement owes exactly 3,000, not the 5,000 dealer margin", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-2881-60", orgId: s.orgId, applicationId });
  ...
  test("money the customer pays the dealership directly cannot become the supplier's debt", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-2913-60", orgId: s.orgId, applicationId });
  ...
  test("a dealership contribution moves its own profit, never the supplier's debt", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-2940-60", orgId: s.orgId, applicationId });
  ...
  test("a receipt above the corrected claim is refused — the old claim was 2,000 larger", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-2970-60", orgId: s.orgId, applicationId });
    ...
    await expect(
      s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-2978-64",
        orgId: s.orgId,
        receivableId: claim._id,
    ...
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-2986-68",
      orgId: s.orgId,
      receivableId: claim._id,
  ...
  test("a partial receipt leaves the remainder outstanding, against the corrected claim", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-2998-60", orgId: s.orgId, applicationId });

    const claim = (await supplierClaimsOf(s)).find((r) => r.status !== "CANCELLED")!;
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-3001-68",
      orgId: s.orgId,
      receivableId: claim._id,
      amount: 1_000,
    });
  ...
  test("cancelling the deal reverses the corrected claim in both the GL and the subledger", async () => {
    ...
    const saleId = await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-3017-75",
      orgId: s.orgId,
      applicationId,
    });
  ...
  test("the cockpit reports the same economic state the subledger and the GL hold", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-3048-60", orgId: s.orgId, applicationId });
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-3049-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: APPROVED * SCALE,
    });
  ...
  test("a direct deal with no approved amount is refused rather than settled at the sale price", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-3140-56", orgId: s.orgId, applicationId })
...
describe("cancelling a deal whose financing evidence cannot be read", () => {
  async function paidDirectDeal(tag: string) {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-3173-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: VEHICLE_PRICE * SCALE,
    });
...
describe("the claim cannot be reopened through a second door", () => {
  test("a currency change between approval and finalization is refused, not silently mixed", async () => {
    ...
    // than stopping.
    await expect(
      s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-3331-56", orgId: s.orgId, applicationId })
  ...
  test("a financed direct sale created outside the application workflow is refused", async () => {
    ...
    await expect(
      s.asUser.mutation(api.sales.create, { idempotencyKey: "t-financedConsignedSettlement.test-3348-43",
        orgId: s.orgId,
        vehicleId: s.vehicleId,
        customerId: s.customerId,
  ...
  test("a CASH direct sale created the same way still completes, at the sale price", async () => {
    ...
    await s.asUser.mutation(api.sales.create, { idempotencyKey: "t-financedConsignedSettlement.test-3371-47",
      orgId: s.orgId,
      vehicleId: s.vehicleId,
      customerId: s.customerId,
      salespersonId: s.userId,
...
describe("automatic commission is based on recognized earnings, not the commercial spread", () => {
  ...
  async function directDealCommissionedAt(tag: string, approvedAmount: number) {
    ...
    const saleId = await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-3443-75",
      orgId: s.orgId,
      applicationId,
    });
    const sale = (await s.t.run((ctx) => ctx.db.get(saleId as never))) as {
...
describe("the sales reports reconcile to the ledger on a financed direct deal", () => {
  async function reportedDeal(tag: string, approvedAmount: number) {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-3561-60", orgId: s.orgId, applicationId });
    return s;
  }
...
describe("financed direct evidence that has gone missing fails closed", () => {
  ...
  async function financedDirectSale(
    ...
    const saleId = (await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-3683-76",
      orgId: s.orgId,
      applicationId,
    })) as never;
  ...
  test("a cash direct sale with no recorded receipt still commissions on the sale price", async () => {
    ...
    const saleId = await s.asUser.mutation(api.sales.create, { idempotencyKey: "t-financedConsignedSettlement.test-4155-62",
      orgId: s.orgId,
      vehicleId: s.vehicleId,
      customerId: s.customerId,
      salespersonId: s.userId,
...
describe("a settlement advice that contradicts the approval", () => {
  async function paidDeal(tag: string, approvedAmount: number) {
    ...
    const saleId = await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-4299-75",
      orgId: s.orgId,
      applicationId,
    });
    return { s, applicationId, saleId };
  ...
  test("is recorded rather than refused, and flagged for reconciliation", async () => {
    ...
    const result = await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4327-90",
      orgId: s.orgId,
      applicationId,
  ...
  test("the reconciliation flag survives the finance gate; the evidence behind it does not", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4372-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
  ...
  test("the people told about a frozen deal are told what froze, and nothing they may not see", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4448-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
  ...
  test("a disbursement on a cancelled deal is still recorded, but nobody is sent to reconcile it", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4537-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-DEAD",
  ...
  test("the application query withholds settlement evidence from a caller who cannot see money", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4581-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
  ...
  test("a role that may confirm a disbursement can still see what it needs to confirm it", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4657-75",
      orgId: s.orgId,
      applicationId,
  ...
  test("the economics query answers the same question the same way", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4706-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
  ...
  test("the applications list redacts what the detail query redacts", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4751-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
  ...
  test("no exported query hands settlement evidence to a sales-only caller", async () => {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-4856-60", orgId: s.orgId, applicationId });
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-4857-75",
      orgId: s.orgId,
      applicationId,
  ...
  test("a sales-only caller learns THAT the supplier was paid, not when or by whom", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5104-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 18_000 * SCALE,
      reference: "WIRE-4471",
  ...
  test("an advice recorded before the status field existed still reads as paid", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5168-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 18_000 * SCALE,
      reference: "WIRE-4471",
  ...
  test("and the same query gives a finance-permitted caller the evidence in full", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5201-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
  ...
  test("and a role holding view:finance receives the evidence in full", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5227-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
  ...
  test("the supplier claim is untouched by the disagreement", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5247-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
    });
  ...
  test("an advice larger than the approval cannot carry the deal to a settled state", async () => {
    ...
    const result = await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5283-90",
      orgId: s.orgId,
      applicationId,
    ...
    const claim = (await supplierClaimsOf(s)).find((row) => row.status !== "CANCELLED")!;
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-5293-68",
      orgId: s.orgId,
      receivableId: claim._id,
      amount: 3_000,
    });
  ...
  test("and correcting the advice back to the approval releases it", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5315-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 20_000 * SCALE,
    });
    const claim = (await supplierClaimsOf(s)).find((row) => row.status !== "CANCELLED")!;
    await s.asUser.mutation(api.supplierReceivables.recordReceipt, { idempotencyKey: "t-financedConsignedSettlement.test-5321-68",
      orgId: s.orgId,
      receivableId: claim._id,
      amount: 3_000,
    });

    await s.asUser.mutation(api.applications.amendSupplierDisbursementAdvice, { idempotencyKey: "t-financedConsignedSettlement.test-5327-79",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 18_000 * SCALE,
  ...
  test("locks cancellation, which the refusal had left wide open", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5340-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
    });
  ...
  test("a funded deal cannot be cancelled even when recording its advice was rejected", async () => {
    ...
    try {
      await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5372-77",
        orgId: s.orgId,
        applicationId,
        disbursedAmountMinor: 17_995 * SCALE,
        reference: "WIRE-4471",
  ...
  test("cancelling the APPLICATION is locked by the same evidence", async () => {
    ...
    try {
      await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5403-77",
        orgId: s.orgId,
        applicationId,
        disbursedAmountMinor: 17_995 * SCALE,
      });
    ...
    await expect(
      s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-5413-61",
        orgId: s.orgId,
        applicationId,
  ...
  test("an agreeing advice is confirmed outright and still locks cancellation", async () => {
    ...
    const result = await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5430-90",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 18_000 * SCALE,
    });
  ...
  test("a mistyped advice can be corrected, and the correction clears the flag", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5453-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
    ...
    const amended = await s.asUser.mutation(api.applications.amendSupplierDisbursementAdvice, { idempotencyKey: "t-financedConsignedSettlement.test-5463-95",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 18_000 * SCALE,
      reference: "WIRE-4471",
  ...
  test("a correction that still disagrees stays flagged rather than being accepted", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5482-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
    });
    ...
    const amended = await s.asUser.mutation(api.applications.amendSupplierDisbursementAdvice, { idempotencyKey: "t-financedConsignedSettlement.test-5490-95",
      orgId: s.orgId,
      applicationId,
  ...
  test("an amendment must say why, and cannot invent an advice that was never recorded", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.amendSupplierDisbursementAdvice, { idempotencyKey: "t-financedConsignedSettlement.test-5507-75",
        orgId: s.orgId,
        applicationId,
        disbursedAmountMinor: 18_000 * SCALE,
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5515-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
    });
    ...
    await expect(
      s.asUser.mutation(api.applications.amendSupplierDisbursementAdvice, { idempotencyKey: "t-financedConsignedSettlement.test-5524-75",
        orgId: s.orgId,
        applicationId,
        disbursedAmountMinor: 18_000 * SCALE,
        reason: "fix",
...
describe("amending one field of a settlement advice", () => {
  ...
  async function advisedDeal(tag: string) {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-5580-60", orgId: s.orgId, applicationId });
    // The advice as it actually arrived: wrong amount, but a real cheque number
    // and a real date, both of which are the evidence.
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5583-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 17_995 * SCALE,
      reference: "WIRE-4471",
  ...
  test("correcting only the amount leaves the reference and the date untouched", async () => {
    ...
    await s.asUser.mutation(api.applications.amendSupplierDisbursementAdvice, { idempotencyKey: "t-financedConsignedSettlement.test-5605-79",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 18_000 * SCALE,
      reason: "Advice re-read: the amount was transposed on entry.",
  ...
  test("the reference and the date can still be corrected when they are given", async () => {
    ...
    await s.asUser.mutation(api.applications.amendSupplierDisbursementAdvice, { idempotencyKey: "t-financedConsignedSettlement.test-5636-79",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 18_000 * SCALE,
      reference: "WIRE-4472",
...
describe("one recognized-earning rule, asked identically by every surface", () => {
  ...
  async function financedDirectDeal(tag: string) {
    ...
    const saleId = (await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-5701-76",
      orgId: s.orgId,
      applicationId,
    })) as never;
    return { s, saleId };
  ...
  test("clearing a wrongly transcribed reference removes it, and the audit says what actually happened", async () => {
    ...
    await s.asUser.mutation(api.applications.confirmSupplierDisbursement, { idempotencyKey: "t-financedConsignedSettlement.test-5959-75",
      orgId: s.orgId,
      applicationId,
      disbursedAmountMinor: 18_000 * SCALE,
  ...
  async function paidDealForClearing(tag: string) {
    ...
    await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-6016-60", orgId: s.orgId, applicationId });
    return { s, applicationId };
  }
});
...
describe("the cockpit's workflow projections", () => {
  test("names the settlement route as outstanding before it offers a close", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-6277-56", orgId: s.orgId, applicationId })
    ).rejects.toThrow(/settlement route/i);
...
describe("the closing matrix c16216 requires", () => {
  ...
  test("cancelling restores the receivable, the consideration and the deposit — each exactly once", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-6701-65",
      orgId: s.orgId,
      applicationId,
  ...
  test("finalizing twice returns the same sale and creates nothing a second time", async () => {
    ...
    const again = await s.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: "t-financedConsignedSettlement.test-6789-74",
      orgId: s.orgId,
      applicationId,
    });
...
describe("a reversal reads back the identity it was recorded under (Sonnet MAX)", () => {
  test("a foreign event sharing the deposit's tuple is not consumed by the cancellation", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7081-65",
      orgId: s.orgId,
      applicationId,
...
describe("a deposit released inside a sale journal stays locked until that journal reverses (CX-4)", () => {
  test("cancelling into a closed period is refused rather than freeing the deposit", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7157-61",
        orgId: s.orgId,
        applicationId,
  ...
  test("two deposits on one sale are refused together, not one at a time", async () => {
    ...
    const { applicationId } = await runDeal(s, {
      ...
      beforeHandover: async (appId) => {
        ...
        await s.asUser.mutation(api.deposits.create, { idempotencyKey: "t-financedConsignedSettlement.test-7201-54",
          orgId: s.orgId,
          quoteId: app!.quoteId,
          amount: 1_000,
        });
    ...
    await expect(
      s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7221-61",
        orgId: s.orgId,
        applicationId,
        reason: "Customer withdrew",
      })
  ...
  test("re-entering on an already-cancelled sale never reaches sale teardown", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7266-65",
      orgId: s.orgId,
      applicationId,
  ...
  test("re-opening a period lets the same cancellation through, unchanged", async () => {
    ...
    await expect(
      s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7304-61",
        orgId: s.orgId,
        applicationId,
        reason: "Customer withdrew",
      })
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7319-65",
      orgId: s.orgId,
      applicationId,
      reason: "Customer withdrew",
    });
...
describe("a stale finance application cannot tear down the sale that replaced it", () => {
  ...
  async function sellAgain(s: Seeded, customerId: Id<"customers">) {
    return await s.asUser.mutation(api.sales.create, { idempotencyKey: "t-financedConsignedSettlement.test-7348-54",
      orgId: s.orgId,
      vehicleId: s.vehicleId,
      customerId,
      salespersonId: s.userId,
  ...
  test("the later sale keeps its car, its cashflow and its ledger — same customer", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7407-65",
      orgId: s.orgId,
      applicationId,
  ...
  test("the later sale keeps everything when a different customer bought it", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7449-65",
      orgId: s.orgId,
      applicationId,
      reason: "Old deal tidied up",
    });
  ...
  test("cancelling the stale application twice changes nothing either time", async () => {
    ...
    for (const attempt of [1, 2]) {
      await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7470-67",
        orgId: s.orgId,
        applicationId,
  ...
  test("a deposit on the stale deal is not handed back a second time", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7517-65",
      orgId: s.orgId,
      applicationId,
      reason: "Old deal tidied up",
    });
  ...
  test("a sale that never completed is never torn down, even reopened behind the seal", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7549-65",
      orgId: s.orgId,
      applicationId,
      reason: "Old deal tidied up",
    });
  ...
  test("cancelling the deal that still owns the car does restore it, once", async () => {
    ...
    await s.asUser.mutation(api.applications.cancelApplication, { idempotencyKey: "t-financedConsignedSettlement.test-7579-65",
      orgId: s.orgId,
      applicationId,
      reason: "Customer withdrew",
    });
