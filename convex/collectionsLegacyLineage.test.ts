...
/**
 * SCRUM-56 — the legacy-row lineage contract.
 *
 * Ported from the evidence branch `agent/scrum-56-lineage-fixtures`, where they
 * were written against the CONTRACT rather than the behaviour and measured at
 * the frozen 603b324fc: F1/F3/F5/F6 failed, F2 passed only because payment
 * order happened to favour it, and F4 passed because `returnClearedCheque`
 * already followed stored lineage.
 *
...
describe("SCRUM-56 lineage contract — fixtures for the redesign", () => {
  ...
  test("F5 cancelling one row leaves the other row's money and the sale's debt intact", async () => {
    ...
    // The row was created through `createReceivable`, so it HAS a twin and that
    // twin must be retired with it. Accepting `null` here would let the
    // stranded-OPEN regression pass whenever a twin happened to be absent.
    expect(state.twinStatus).toBe("CANCELLED");
  });
  ...
  test("F7 the symmetric case: refunding B reverses B's money, not A's", async () => {
    ...
    // F1 refunds the OLDER row's payment, F7 the NEWER. Under newest-first one
    // of the two passes by luck; only lineage passes both.
    await refund(asUser, asApprover, orgId, rowB, 11000);

    expect(await appliedMinorForRow(t, rowB)).toBe(0);
    expect(await appliedMinorForRow(t, rowA)).toBe(11000 * 1000);
  });

  test("F8 repeated partial refunds keep the remainder attributable after each", async () => {
    const { t, orgId, customerId, vehicleId, userId, asUser, asApprover } = await setup();
    const saleId = await completeSale(asUser, orgId, customerId, vehicleId);
    const rowA = await addRow(t, orgId, customerId, userId, saleId, 11000, "A");
    const rowB = await addRow(t, orgId, customerId, userId, saleId, 11000, "B");
    ...
    // Each refund reverses an allocation and re-allocates a remainder under a
    // NEW id. If lineage rode on the stored allocation pointer it would name a
    // reversed row by the second pass, so the third would find nothing.
    await refund(asUser, asApprover, orgId, rowA, 3000);
    expect(await appliedMinorForRow(t, rowA)).toBe(8000 * 1000);

    await refund(asUser, asApprover, orgId, rowA, 3000);
    expect(await appliedMinorForRow(t, rowA)).toBe(5000 * 1000);

    await refund(asUser, asApprover, orgId, rowA, 5000);
    expect(await appliedMinorForRow(t, rowA)).toBe(0);

    // B financed none of it.
    expect(await appliedMinorForRow(t, rowB)).toBe(11000 * 1000);
  });

  test("F9 a payment link settles the sale invoice, not the row's obsolete twin", async () => {
    const { t, orgId, customerId, vehicleId, asUser } = await setup();
    const saleId = await completeSale(asUser, orgId, customerId, vehicleId);

    // Created through the real mutation so the row owns a `legacy_receivable`
    // twin — the document a payment link used to settle while cash and cheque
    // settled the sale invoice.
    const rowId = await asUser.mutation(api.collections.createReceivable, {
      orgId, customerId, saleId, sourceType: "INTERNAL_INSTALLMENT",
      title: "Link", amount: 8000, dueDate: Date.now() + 86_400_000,
    });

    const intentId = await asUser.mutation(api.paymentIntents.create, {
      orgId, customerId, receivableId: rowId, saleId,
      amountMinor: 8000 * 1000, currency: "JOD", provider: "stripe",
      externalId: "pi_lineage_1",
    });
    ...
    const state = await t.run(async (ctx: any) => {
      ...
      return {
        onInvoice: active
          .filter((a: any) => a.receivableDocumentId === sale.canonicalReceivableDocumentId)
          .reduce((s: number, a: any) => s + a.amountMinor, 0),
        onTwin: active
          .filter((a: any) => a.receivableDocumentId === row.canonicalReceivableDocumentId)
          .reduce((s: number, a: any) => s + a.amountMinor, 0),
      };
    });

    expect(state.onInvoice).toBe(8000 * 1000);
