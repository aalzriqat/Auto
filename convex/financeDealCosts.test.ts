...
async function addFee(
  ...
  return await seed.asUser.mutation(api.financeDealCosts.recordDealFee, { idempotencyKey: "t-financeDealCosts.test-84-73",
    orgId: seed.orgId,
...
describe("a retried submit does not spend money twice", () => {
  ...
  test("the same key replays a reimbursement instead of paying it again", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-136-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
...
describe("employee custody", () => {
  test("closes the identity when the balance is returned", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-325-88",
      orgId: seed.orgId,
      applicationId: seed.applicationId,
    ...
    await seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-337-76",
      orgId: seed.orgId,
      custodyId,
      kind: "RETURNED",
      amountMinor: jod(50),
  ...
  test("an employee who spent their own money is owed it, not charged for it", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-351-88",
      orgId: seed.orgId,
      applicationId: seed.applicationId,
      userId: seed.employeeId,
      issuedMinor: jod(700),
  ...
  test("a debt to the employee is not settled until it is actually paid", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-378-88",
      orgId: seed.orgId,
      applicationId: seed.applicationId,
      userId: seed.employeeId,
      issuedMinor: jod(700),
    ...
    await seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-396-76",
  ...
  test("totals are a sum of movements, so a correction stays visible", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-409-88",
      orgId: seed.orgId,
      applicationId: seed.applicationId,
      userId: seed.employeeId,
      issuedMinor: jod(700),
    });
    await seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-415-76",
  ...
  test("an unbalanced record can only be closed as an explicit write-off", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-438-88",
      orgId: seed.orgId,
      applicationId: seed.applicationId,
      userId: seed.employeeId,
      issuedMinor: jod(700),
  ...
  test("custody cannot be handed to someone outside the organization", async () => {
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-470-66",
        orgId: seed.orgId,
        applicationId: seed.applicationId,
        userId: outsider,
        issuedMinor: jod(700),
  ...
  test("custody cannot be handed to somebody who is being offboarded", async () => {
    ...
    // an active one.
    await expect(
      seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-498-66",
        orgId: seed.orgId,
        applicationId: seed.applicationId,
        userId: seed.employeeId,
        issuedMinor: jod(700),
  ...
  test("a second open record for the same person on the same deal is refused", async () => {
    const seed = await seedDeal();
    await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-509-70",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });

    await expect(
      seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-515-66",
        orgId: seed.orgId, applicationId: seed.applicationId,
  ...
  test("a fee cannot be attached to another deal's custody record", async () => {
    ...
    const otherCustodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-545-93",
      orgId: seed.orgId,
...
describe("a closed record cannot go quietly wrong afterwards", () => {
  async function settledCustody(seed: Seed) {
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-565-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
...
describe("money can only move in directions that are true", () => {
  test("a cost somebody else paid cannot be charged to an employee's custody", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-698-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
  ...
  test("a duplicate reimbursement is surfaced, not clamped away", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-717-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
    await addFee(seed, { actualAmountMinor: jod(750), paidBy: "EMPLOYEE", custodyId });
    ...
    for (let i = 0; i < 2; i += 1) {
      await seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-725-78",
        orgId: seed.orgId, custodyId, kind: "REIMBURSED", amountMinor: jod(50),
      });
    }
  ...
  test("a return larger than the advance is refused rather than inverting the balance", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-742-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-750-72",
  ...
  test("a debt owed to an employee cannot be written off unpaid", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-758-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
    await addFee(seed, { actualAmountMinor: jod(750), paidBy: "EMPLOYEE", custodyId });
  ...
  test("a mistyped issuance is corrected by a reversal, not by a fictitious return", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-778-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
    ...
    await seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-792-76",
      orgId: seed.orgId, custodyId, kind: "REVERSAL",
      reversesEntryId: entryId, amountMinor: jod(7000),
      note: "Mistyped: should have been 700.",
    });
    await seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-797-76",
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-813-72",
        orgId: seed.orgId, custodyId, kind: "REVERSAL",
        reversesEntryId: entryId, amountMinor: jod(7000),
      })
  ...
  test("reversing an issuance cannot leave more returned than was ever issued", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-822-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
    const entryId = await seed.t.run(async (ctx) => {
    ...
    await seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-833-76",
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-843-72",
        orgId: seed.orgId, custodyId, kind: "REVERSAL",
  ...
  test("an issuance already settled against cannot be reversed out from under the settlement", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-856-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
    const entryId = await seed.t.run(async (ctx) => {
    ...
    await addFee(seed, { actualAmountMinor: jod(750), paidBy: "EMPLOYEE", custodyId });
    await seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-869-76",
      orgId: seed.orgId, custodyId, kind: "REIMBURSED", amountMinor: jod(50),
    });
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-882-72",
        orgId: seed.orgId, custodyId, kind: "REVERSAL",
        reversesEntryId: entryId, amountMinor: jod(700),
      })
  ...
  test("a reversal can only cancel a movement on its own custody record", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-895-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
    ...
    const otherCustodyId = await seed.asUser.mutation(
      ...
      { idempotencyKey: "t-financeDealCosts.test-913-7",
        orgId: seed.orgId, applicationId: seed.applicationId,
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-927-72",
        orgId: seed.orgId, custodyId, kind: "REVERSAL",
  ...
  test("a reversal must cancel the whole movement, and only a reversal may name one", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-936-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
    const entryId = await seed.t.run(async (ctx) => {
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-951-72",
        orgId: seed.orgId, custodyId, kind: "REVERSAL",
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-958-72",
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordCustodyMovement, { idempotencyKey: "t-financeDealCosts.test-965-72",
  ...
  test("replacing a cost's receipts deletes the ones it drops", async () => {
    ...
    const feeId = await seed.asUser.mutation(api.financeDealCosts.recordDealFee, { idempotencyKey: "t-financeDealCosts.test-976-82",
      orgId: seed.orgId,
      applicationId: seed.applicationId,
      feeType: "LICENSING",
      paidBy: "DEALER",
  ...
  test("custody cannot be issued to yourself", async () => {
    const seed = await seedDeal();
    await expect(
      seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-1002-66",
        orgId: seed.orgId, applicationId: seed.applicationId,
...
describe("reconciled work is not destroyable from below", () => {
  ...
  test("reopening a written-off record keeps the reason the loss was accepted for", async () => {
    const seed = await seedDeal();
    const custodyId = await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-1099-88",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
    await addFee(seed, { actualAmountMinor: jod(600), paidBy: "EMPLOYEE", custodyId });
...
describe("the legal invoice and accounting classification", () => {
  ...
  test("an open custody record blocks classification", async () => {
    ...
    await seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-1194-70",
      orgId: seed.orgId, applicationId: seed.applicationId,
      userId: seed.employeeId, issuedMinor: jod(700),
    });
...
describe("tenancy", () => {
  test("another organization's deal is not reachable", async () => {
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.recordDealFee, { idempotencyKey: "t-financeDealCosts.test-1375-64",
        orgId: seed.orgId,
        applicationId: otherApplicationId,
        feeType: "LICENSING",
        paidBy: "DEALER",
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-1387-66",
        orgId: seed.orgId,
        applicationId: otherApplicationId,
        userId: seed.employeeId,
        issuedMinor: jod(100),
...
describe("input validation", () => {
  test("rejects NaN and negative money, which pass Convex's own validator", async () => {
    ...
    await expect(
      seed.asUser.mutation(api.financeDealCosts.openDealCustody, { idempotencyKey: "t-financeDealCosts.test-1406-66",
        orgId: seed.orgId,
        applicationId: seed.applicationId,
        userId: seed.employeeId,
...
describe("classification establishes the remittance from what the company actually withholds", () => {
  ...
  async function classifyWithFee(
    ...
    const feeId = await seed.asUser.mutation(api.financeDealCosts.recordDealFee, { idempotencyKey: "t-financeDealCosts.test-1446-82",
      orgId: seed.orgId,
      applicationId: seed.applicationId,
      feeType: "LICENSING",
      paidBy: "DEALER",
