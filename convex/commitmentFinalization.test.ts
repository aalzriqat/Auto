 ...
 *
 * ⚠️ WHICH FINANCE STATUSES HOLD A CAR IS ANSWERED ONCE, by the exported
 * `IN_FLIGHT_FINANCE_STATUSES`. It began as a local `const` inside
 * `createFromQuote`'s handler, and M3's first attempt ADDED the shared constant
 * without deleting the local one — shipping the two independently-maintained
 * lists it claimed to have removed, which both reviewer seats caught. The local
 * copy is gone; acquisition and release now read the same array and cannot
 * drift. Same shape as the six acquisition writers that each decided evidence
 * kind for themselves.
 ...
 *
 *   PART X held five contracts pinning the gap as it existed BEFORE M3 — a
 *   completed sale leaving the root OPEN, a release doing the same, a
 *   cancellation not freeing the car, a committed car being soft-deletable, and
 *   an expired reservation locking the car forever. M3 makes all five false, so
 *   the M3 commit DELETED them. They are named here rather than kept, because a
 *   contract asserting behaviour the system no longer has is not evidence.
 *
...
async function completeQuote(seed: Seed, quoteId: Id<"quotes">) {
  ...
  return (await seed.asUser.mutation(api.sales.completeFromQuote, { idempotencyKey: crypto.randomUUID(),
    orgId: seed.orgId,
    quoteId,
  })) as Array<Id<"sales">>;
}
...
async function directSale(
  ...
  return (await seed.asUser.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
    orgId: seed.orgId,
    vehicleId,
    customerId,
...
async function releaseDeposit(
  ...
  return await seed.asManager.mutation(api.deposits.release, { idempotencyKey: crypto.randomUUID(),
    orgId: seed.orgId,
    depositId,
    resolution,
...
async function cancelApplication(seed: Seed, applicationId: Id<"financeApplications">) {
  return await seed.asManager.mutation(api.applications.cancelApplication, { idempotencyKey: crypto.randomUUID(),
    orgId: seed.orgId,
    applicationId,
...
describe("P2-F M3 finalization barrier — CONSUME", () => {
  ...
  test("F.9a2 DOOR 1 with NO LINEAGE AT ALL is refused on a held car", async () => {
    ...
    await expect(
      seed.asUser.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
        orgId: seed.orgId,
        vehicleId: v,
  ...
  test("F.9e DOOR 4 finalizeDeal REFUSES once the car has legitimately moved to another deal", async () => {
    ...
    await expect(
      seed.asUser.mutation(api.applications.finalizeDeal, { idempotencyKey: crypto.randomUUID(),
        orgId: seed.orgId,
        applicationId,
      }),
...
describe("P2-T the tenant boundary", () => {
  test("T.1 a sale from ANOTHER DEALERSHIP never becomes this root's provenance", async () => {
    ...
    const foreignSale = await other.asUser.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
      orgId: other.orgId,
  ...
  test("T.4 PUBLIC PATH — a foreign completion refuses and closes nothing", async () => {
    ...
    await expect(
      other.asUser.mutation(api.sales.create, { idempotencyKey: crypto.randomUUID(),
        orgId: other.orgId,
        vehicleId: car,
        customerId: other.customerId,
        salespersonId: other.userId,
