...
- **[HRN-1]** A validator must never return PASS for a property it did not measure. Absent, empty or unparseable evidence is UNAVAILABLE.
- **[HRN-2]** A guard must decide from INVOCATION structure, never from raw substring presence. Documentation, fixtures, comments, quoted strings, commit messages, Jira/Slack text and reviewer packets must be able to discuss a dangerous command.
