...
// ISO 3779 — character at VIN position 10 (0-indexed: 9) encodes the model year.
// The alphabet repeats every 30 years (A=1980/2010, B=1981/2011, …).
...
// Title-case brand names: acronyms (≤3 chars) stay ALL-CAPS, longer words get capitalised.
// "GREAT WALL" → "Great Wall"   "BYD" → "BYD"   "TOYOTA" → "Toyota"
...
// Strip legal suffixes from a WMI manufacturer string so we get the brand name.
// "BYD AUTO CO., LTD" → "BYD"   "GREAT WALL MOTOR CO.,LTD" → "GREAT WALL"
function cleanMfrName(mfr: string): string {
  const cleaned = mfr
    ...
    .replace(/\s+/g, " ")
    .trim();
  return cleaned || mfr.split(/\s+/)[0]; // fall back to first word if everything got stripped
}
...
export function VehicleDialog({ open, onOpenChange, vehicle, canCreate = false, canEdit = false }: VehicleDialogProps) {
  ...
  const handleDecodeVIN = async () => {
    ...
    setIsDecoding(true);
    try {
      // Fire both lookups in parallel — WMI is the global manufacturer registry (ISO 3780)
      // and is more reliable for Make on non-US VINs; full VIN decode gives model/trim/fuel/year.
      ...
      // Make: WMI wins (correct for every globally registered manufacturer),
      // fall back to the full VIN decode result.
      ...
      // Model / trim / fuel — only available from the full VIN decode.
      const model   = (v.Model || v.Series || "").trim();
      const trim    = v.Trim?.trim() ?? "";
      const fuelRaw = v.FuelTypePrimary?.toLowerCase() ?? "";

      // Year: VIN decode first, then decode it directly from VIN position 10 (ISO 3779).
