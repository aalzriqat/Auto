...
- [x] **Phase 61 — Trust passport (v1, self-reported).** Widened `vehicles` with `inspectionStatus`/`accidentDisclosed`/`ownerCount`/`dealerGuarantee` (all optional). Trust info panel on marketplace vehicle cards. **Dealer-facing self-service form added** (2026-07-12): a "Trust Passport" section in `components/vehicles/VehicleDialog.tsx` lets a dealer set inspection status (NONE/SELF_REPORTED only — `PARTNER_VERIFIED` stays locked/admin-only, reserved for a future partner-API integration), accident history (tri-state: not disclosed/no accidents/accident disclosed), owner count, and dealer guarantee, flowing through `vehicles.create`/`update` directly or `vehicleEdits.requestCreate`/`requestUpdate` + approval for non-privileged roles, same as every other vehicle field.
