...
- **SCRUM-223 — the `COLLECTION_PAYMENT` migration producer is retired.** A legacy collection row is `SKIP`-ped with disposition `RETIRED_COLLECTION` and `classificationReason: "collection_producer_retired"`. This is deliberately a *different* disposition from `UNMAPPED`: the category is not merely unmapped, its authority to originate a receipt has been withdrawn, because the modern producers already post the receipt sourced from `collectionPayments` and the migration's duplicate probe (which looks for `sourceType: "transactions"`) cannot see that event. Migrating such a row minted a second journal for one economic receipt.
  - Consequence for `migrationGapAnalysis.migrationProgress`: it is `Math.min(100, Math.round((glEventCount / legacyCount) * 100))` (`accountingMigration.ts:323`), and its numerator is **uncorrelated with its denominator**. `glEventCount` (`accountingMigration.ts:275-282`) counts every `accountingEvent` whose `sourceType` is `transactions`: no status filter, no de-duplication by `sourceId`, and no check that the event belongs to a row still counted as legacy.

    Retiring a row therefore means only that **retirement itself mints no new event**. It does *not* mean the row contributes nothing, because a row classified retired today may already carry a POSTED event minted by the pre-retirement producer. This branch's own fixture builds exactly that org (`accountingCollectionProducerRetirement.test.ts:387`): one legacy row, one historical event, `migrationProgress = 100`, and the row still classified `RETIRED_COLLECTION`. Rounding and the `Math.min` cap hide the movement from the other direction too — 999 migrated rows beside one never-migrated retired row still reads `Math.round(99.9) = 100`. **`migrationProgress` cannot identify retired rows in either direction** and must not be used as evidence about them. The authoritative per-row signal is the `RETIRED_COLLECTION` disposition returned by `auditLegacyTransactions` and `migrateUnpostedTransactions`.
    - ⚠️ Note the contrast with `signOffCutover`, which *does* filter `status === "POSTED"` when building its migrated set (`accountingCutover.ts:824-827`). The two counts are deliberately different and only one of them is a gate. Do not reason from one to the other.
  ...
  - ⚠️ **Consequence, stated plainly rather than left implied: `signOffCutover` and `compareLegacyToGL` cannot reach a passing state for an org holding permanently-unmigratable legacy rows.** `signOffCutover` throws while `unmigratedTransactionCount > 0`, and a retired row can never acquire a POSTED event sourced from `transactions` — that is precisely what retirement means. This is **not new with SCRUM-223**: `collections.ts` already writes an ongoing `category: "REFUND"` legacy row that no mapping covers, so any org that has processed one refund is already permanently blocked on protected main today.
    ...
    ⚠️ **And the scope is not a legacy backlog — it is unconditional and ongoing.** `recordPayment` (`collections.ts:1076`) and `clearCheque` (`:1376`) each write their legacy `COLLECTION_PAYMENT` row on **every** call, outside any conditional — the insert sits after the `if (receivable)` block, not inside it. So this is not a subset of orgs carrying historical rows: **a brand-new org becomes permanently unable to reach `signOffCutover` the moment it records its first collection payment through the current, correct flow**, and stays blocked. Read that as the real urgency of the SCRUM-237 correlation work; do not read it as "clean orgs are exempt."

      They are **not**, however, the only writers of that legacy category, and an earlier revision of this document wrongly said they were. `transactions.add` is a public `mutation` (`transactions.ts:213`) whose validator accepts `category: "COLLECTION_PAYMENT"` (`:222`) behind `MANAGE_FINANCE` alone, and the shipped mobile Accounting form offers that category directly (`apps/mobile/src/features/workspace/modules/accounting.tsx:35`). A row written that way has **no** `collectionPayments` counterpart at all. `transactions.update` (`:285`, category at `:295`, same permission at `:305`) is the second such surface, and it is the worse of the two: it retags an *existing* row into the category after the fact, through the same mobile form (`accounting.tsx:151` renders one category control for both create and edit). Those two are the caller-controlled writers; every other production `db.insert("transactions")` site passes a hard-coded non-collection category.

      The retag case matters specifically to SCRUM-237. A row recategorized this way keeps whatever `idempotencyKey` it was born with — inherited from an unrelated original category — so it presents a **plausible-looking correlation token that correlates to nothing**. For the identity work that is worse than the `transactions.add` case, which is a clean absence: absence is detectable, a false positive is not. Neither row can reach the retirement logic incorrectly (both classify by *current* category, and `update` fires no hook, no outbox entry and no journal), so SCRUM-223's own guarantee is unaffected either way. That writer is out of scope for SCRUM-223 — it is the writable-shadow-ledger defect tracked as SCRUM-53 — but it is precisely why the correlation described in the next paragraph cannot be assumed to exist for every retired row.
    ...
    - It is deliberately **not** closed here. The obvious fix — excluding retired rows from the unmigrated count — would let a genuinely orphaned legacy collection row (one with no `collectionPayments` counterpart) be silently excused, turning a workflow dead-end into a false attestation on a financial gate, which is worse. Closing it safely requires positively verifying that each retired row's economic effect is captured elsewhere.

      A **partial** correlation handle does exist, and an earlier revision of this document wrongly denied it outright: `recordPayment` and `clearCheque` write the *same* `args.idempotencyKey` onto both the `collectionPayments` row and the legacy `transactions` row (`collections.ts:1065` and `:1085`; `:1366` and `:1385`). It is not sufficient to close the gate on, for three reasons. `idempotencyKey` is **optional** on the legacy row (`transactions.ts:230`), so rows may carry none. A row written through `transactions.add` has no `collectionPayments` counterpart to correlate with at all. And shared-key equality is a convention observed by two call sites, not an indexed or enforced relation — Convex has no unique indexes, so nothing constrains a third writer to reuse the value, or to reuse it uniquely. Promoting that handle into a durable, enforced occurrence identity is exactly the canonical identity owned by SCRUM-237, so this stays follow-up rather than being patched here.
