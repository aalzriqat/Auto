 ...
 *
 * Destructive lifecycle SEQUENCING belongs to SCRUM-231. It is not implemented
 * here, and this file must never be read as standing in for it.
 *
 * The runtime guarantee for the customer-merge seam was SCRUM-250's. That ticket
 * is OUT of the release candidate and must not be restored to any activation
 * chain: the seam it was to fence no longer exists (SCRUM-314), so there is no
 * merge path left to refuse.
 *
 ...
 *
 * ## ⚠️ R1, R2 and R3 ARE RETIRED ON THIS BRANCH — read `MERGE_SUBJECT_RETIRED`
 *
 * SCRUM-314 deleted the customer-merge product seam, which is the ONLY subject
 * those three rules read. Owner ruling `SCRUM-313` Option 1 retires them with it
 * rather than resurrecting deleted code to give a guard something to read.
 *
 * This is an INTEGRATION ADAPTATION, not a successful execution of the original
 * R1/R3 oracle, and not a relaxation of R4/R5/R6. A RETIRED rule proves nothing,
 * is printed as such, and is excluded from the executed-rule counts. The rule
 * bodies below are left intact and re-arm automatically if the module returns.
 *
 * The property R1/R3 guarded — that a generic customer-rewriting registry must
 * never rewrite the three authority tables — now holds BY CONSTRUCTION, more
 * strongly than a source guard could establish, because no such registry exists.
 *
...
export type Verdict = "SATISFIED" | "VIOLATED" | "INACTIVE" | "RETIRED";
...
export interface InvariantResult {
  ...
  /** Present only when RETIRED: why the rule's subject no longer exists. */
...
/**
 * A rule whose SUBJECT was deliberately deleted from the product.
 *
 * ⚠️ RETIRED IS NOT A PASS, AND IT IS NOT AN INACTIVE EITHER.
 *
 * INACTIVE means "the subject does not exist YET" — a ratchet waiting to arm.
 * RETIRED means "the subject was REMOVED ON PURPOSE and is not coming back", so
 * the original oracle can never run again and must not be reported as though it
 * had. A RETIRED result carries no `proved` claim of its own: it records what the
 * rule USED to prove and why that question stopped having a subject.
 *
 * The retirement is **conditional, not unconditional**. It is emitted only while
 * the subject module is genuinely absent; the original certified rule bodies are
 * left intact below and reactivate automatically the moment the module is
 * readable again. That is what keeps this from becoming the fail-open hiding
 * place the whole programme exists to prevent: reintroducing the deleted surface
 * does not inherit the retirement, it re-arms the rule.
 */
function retired(
  id: string,
  title: string,
...
/**
 * Why R1, R2 and R3 are RETIRED on this branch, recorded once.
 *
 * SCRUM-314 deleted the customer-merge product seam outright — `mergeHelpers.ts`,
 * `CUSTOMER_REFERENCING_TABLES`, `CUSTOMER_NON_REASSIGNABLE_TABLES`,
 * `customers.mergeCustomers` / `previewMerge` / `findMergeCandidates`, and the
 * frontend dialog. All three of those rules read that module and nothing else.
 *
 * SCRUM-238 was authored while the seam still existed, and its own suite pinned
 * the fail-closed polarity: deleting `mergeHelpers.ts` VIOLATES R1 and R3 rather
 * than satisfying them. That polarity is correct and was deliberately not
 * inverted. What changed is the question, not the answer: the property "no
 * generic customer-rewriting registry rewrites these three tables" now holds BY
 * CONSTRUCTION, more strongly than R1/R3 could ever verify, because no such
 * registry exists anywhere in `convex/` to be checked.
 *
 * A guard that can only say "cannot prove" because its subject was intentionally
 * deleted must not force that subject back into existence. Owner ruling
 * `SCRUM-313` (Option 1) retires R1 and R3 with their subject. R2 is retired on
 * the same evidence and by the same ruling's own qualifier — its subject,
 * `CUSTOMER_NON_REASSIGNABLE_TABLES`, lives in that same deleted module, so it is
 * not one of the surviving checks the ruling ordered kept.
 *
 * R4, R5 and R6 read `adminData.ts`, the two destructive lifecycle manifests and
 * every convex module respectively. None of those subjects moved, so none of
 * those rules is touched.
 */
const MERGE_SUBJECT_RETIRED =
  `SCRUM-314 deleted the customer-merge seam: ${MERGE_HELPERS_MODULE} does not exist, ` +
  `so ${REWRITE_REGISTRY} and ${SEALED_CLASSIFICATION} have no declaration anywhere. ` +
  "Retired by owner ruling SCRUM-313 Option 1 — an integration adaptation, NOT a run of the original oracle.";
...
export function checkStructuralRatchet(source: SourceProvider): RatchetReport {
  ...
  if (mergeText === null) {
    // The subject was deleted on purpose. See MERGE_SUBJECT_RETIRED.
    invariants.push(
      retired(
        "R1",
        ...
        MERGE_SUBJECT_RETIRED
      )
    );
  } else if (registry === null || registry.kind !== "ok") {
    invariants.push(
      violated(
        "R1",
        "authority tables are absent from the customer-merge REWRITE registry",
  ...
  if (mergeText === null) {
    // Retired with R1 and R3: the sealed classification lives in the same
    // deleted module. See MERGE_SUBJECT_RETIRED.
    invariants.push(
      retired(
        "R2",
  ...
  if (mergeText === null) {
    // Retired with R1: there are no registry finders left to couple-check.
    // See MERGE_SUBJECT_RETIRED.
    invariants.push(
      retired(
        "R3",
...
export function formatRatchetReport(report: RatchetReport): string {
  ...
  for (const invariant of report.invariants) {
    ...
    } else if (invariant.verdict === "RETIRED") {
      lines.push(`    used to prove: ${invariant.proved}`);
      lines.push(`    retired because: ${invariant.retiredBecause ?? "unknown"}`);
      lines.push("    NOT EXECUTED — this rule proved nothing on this run.");
    } else {
  ...
  lines.push(
    "  - There is no customer-merge path to refuse: SCRUM-314 deleted the seam, so R1,"
  );
  lines.push(
    "    R2 and R3 are RETIRED with it and SCRUM-250 is OUT. Nothing here re-proves that"
  );
  lines.push("    absence — it is a precondition of the retirement, not a claim of this run.");
  lines.push(
  ...
  lines.push(
    "  - A RETIRED verdict means the rule's subject was DELETED ON PURPOSE, so the rule"
  );
  lines.push(
    "    did not execute at all. It is not a pass either, and it must never be counted"
  );
  lines.push("    as one. It is an integration adaptation, recorded rather than hidden.");
  lines.push("");

  const retiredIds = report.invariants
    .filter((invariant) => invariant.verdict === "RETIRED")
    .map((invariant) => invariant.id);
  const executed = report.invariants.filter((invariant) => invariant.verdict !== "RETIRED");
  const satisfiedCount = executed.filter((i) => i.verdict === "SATISFIED").length;
  lines.push(
    `EXECUTED: ${executed.length} rule(s) — ${satisfiedCount} SATISFIED, ` +
      `${executed.filter((i) => i.verdict === "VIOLATED").length} VIOLATED, ` +
      `${executed.filter((i) => i.verdict === "INACTIVE").length} INACTIVE.`
  );
  if (retiredIds.length > 0) {
    lines.push(
      `RETIRED (not executed, not counted as passing): ${retiredIds.join(", ")}.`
    );
  }
  lines.push("");
