...
describe("the analyzer's coverage does not shrink silently", () => {
  ...
  //
  // `financeDealCosts.planCustodyHandler` and `financeDealCosts.setFeeCustody`
  // (AF-80 employee cash custody accounting) — 490 → 492 total, 318 → 320
  // analysed. Both take `orgId` plus a caller-supplied id (`applicationId`,
  // `feeId`) read through `requireOwnedRow` after `requireTenantAuth`;
  // `setFeeCustody`'s optional second row id (`custodyId`) goes through the
  // shared `resolveFeeCustody`, which is its own `requireOwnedRow` plus the
