...
import { dateLabel, maybeText, useGenericError, PrimaryButton, SegmentedControl, FormField, SelectField, FormModal, RecordCard, ModuleList } from "./moduleShared";
...
export function FeedbackModule({ orgId }: { orgId: string }) {
  ...
  return (
    <>
      <ModuleList
        data={feedback}
        emptyLabel={locale === "ar" ? "لا توجد ملاحظات." : "No feedback found."}
        keyExtractor={(item: MobileFeedback) => item._id}
        header={
          <>
        ...
        renderItem={(item: MobileFeedback) => (
          <RecordCard>
