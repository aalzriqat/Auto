    ...
    <ModuleScroll>
      ...
      {filtered.length ? filtered.map((customer) => (
        <RecordCard key={customer._id}>
          ...
          <Text style={styles.recordMeta}>{customer.phone || customer.email || "-"}</Text>
          <Text style={styles.recordMeta}>{customer.address || customer.source || "-"}</Text>
    ...
    <ModuleScroll>
      ...
      {filtered.length ? filtered.map((lead) => (
        <RecordCard key={lead._id}>
          ...
          <Text style={styles.recordMeta}>{lead.vehicleSummary || (locale === "ar" ? "بدون سيارة" : "No vehicle")}</Text>
          <Text style={styles.recordMeta}>{locale === "ar" ? "المسؤول" : "Owner"}: {lead.assignedUserName || "-"}</Text>
          <View style={styles.cardActions}>
      ...
      <LoadMoreFooter loadMore={loadMore} status={status} />
      <FormModal title={locale === "ar" ? "فرصة جديدة" : "New lead"} visible={open} onClose={() => setOpen(false)}>
        ...
        <SelectField label={locale === "ar" ? "المرحلة" : "Stage"} value={form.stage} options={stageOptions.filter((option) => option.value !== "ALL").map((option) => ({ label: option.label, value: option.value }))} onChange={(stage) => setForm((prev) => ({ ...prev, stage: stage as MobileLeadStage }))} />
        <FormField multiline label={locale === "ar" ? "ملاحظات" : "Notes"} value={form.notes} onChangeText={(notes) => setForm((prev) => ({ ...prev, notes }))} />
        <PrimaryButton disabled={saving} label={saving ? (locale === "ar" ? "جاري الحفظ..." : "Saving...") : (locale === "ar" ? "حفظ" : "Save")} onPress={save} />
      </FormModal>
    </ModuleScroll>
